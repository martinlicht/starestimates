...
\newcommand{\tcfsays}[1]{{\color{blue} [TCF: #1]}}
...
These quantify the fundamental stability properties of \tcf{certain} partial differential equations
and enter the stability and convergence theory of numerical methods. 
Upper bounds of the Poincar\'e--Friedrichs constants also provide lower bounds of the eigenvalues of the associated Laplacians \tcf{\sout{and the eigenvalues of finite element matrices}} \tcfsays{This is not true in general I believe}.
...
This manuscript contributes to the theory of computable estimates for Poincar\tcf{\'e}--Friedrichs inequalities for the differential operators of vector calculus. 
How to construct potentials for the gradient over triangulated domains is well-documented in the literature. 
\tcf{Here, we extend this idea} % We apply this construction principle
to the curl and divergence operators, as well as the exterior derivative.
...
for example, local triangulations around a simplex \tcfsays{Is it really true if the simlex is of highest dimension, e.g., a tetrahedron in 3D?} within a larger triangulation (the so-called local patches or stars) are shellable in dimensions two and three. 
...
Attention has been given to \emph{linear} potential operators instead of the norm-minimizing nonlinear potential~\eqref{math:intro:gradientpotential}. 
...
Our objective is to find bounded potentials \tcf{(i.e. left inverses)} for the operators 
\begin{align*}
...
We are therefore interested in any potential operators 
\begin{align}
...
This formalism is independent of the spatial dimension and highlights \tcf{\sout{extends}} the underlying geometric structures common to the gradient, curl, and divergence operators in three space dimensions.
% This formalism is independent of the spatial dimension and highlights underlying geometric structures common to the gradient, curl, and divergence operators.In this setting, the common writing of the Poincar\'e--Friedrichs inequalities we are interested in is just as~\eqref{math:PF_gradient_1}: 
\tcf{F}or every $u \in W^{p}\Alt^{k}(\Omega)$, there exists $w \in W^{p}\Alt^{k}(\Omega)$ with $\cartan w = \cartan u$ and such that (all the notation is fixed in detail in Section~\ref{section:calculus} later)
...
These Poincar\'e-type inequalities are often derived via non-constructive arguments, such as the Rellich embedding theorem \tcfsays{I believe this is often refered to as the Tartare-Petree theorem},
...
with homogeneous Dirichlet boundary conditions along $\partial \Omega$ implies~\tcf{\eqref{math:intro:Wdiv:pf}}
\tcf{by}  %% via
duality. However, that easy duality argument, which yields an explicit upper bound proportional to the domain diameter, seems inherently restricted to $p=2$. 
...
We refer to~\cite[Equation~(5)]{Fried_diff_forms_55}, \cite[Equation~(2)]{Gaff_Hilbert_harm_55}, \cite{Web_compact_Maxw_80} as well as \cite[Lemmas~3.4 and~3.6]{Gir_Rav_NS_86}, \cite[Proposition~7.4]{Fer_Gil_Maxw_BC_97}, and the references therein. 
...
Unfortunately, no results as those of~\cite{guerini2004eigenvalue} are known over bounded convex Lipschitz domains and for general Lebesgue exponents $1 \leq p \leq \infty$ \tcfsays{We might want to obtain with ML's recent preprint.}. 
...
Poincar\'e--Friedrichs inequalities actually hold over any star-shaped open bounded sets~\cite[Theorem~3.1]{hurri1988poincare} \tcfsays{I don't exactly get the point of this sentence.}. 
Polynomial interpolation estimates already imply the \tcf{gradient} Poincar\'e--Friedrichs inequality~\cite{brenner2008mathematical,ern2021finite}. 
...
In principle, the operator norms of those potential operators as mapping between Lebesgue spaces of differential forms 
...
Recall that the main objective of this manuscript is bounding Poincar\'e--Friedrichs constants over
\tcf{domains with shellable triangulations, with a key application being} local stars within triangulated domains. 
...
The method is known in the finite element literature~\cite{ern2021finite} \tcfsays{Could you reference a particular theorem or chapter?}. It was previously used in the context of finite volume methods~\cite{Eym_Gal_Her_00}, broken (weakly continuous) Sobolev spaces~\cite{vohralik2005discrete}, or more recently in continuous--discrete comparison results~\cite{Brae_Pill_Sch_p_rob_09, ern2020stable, Chaum_Voh_p_rob_3D_H_curl_24,  Voh_loc_glob_H1_24}. 
This sequential procedure applies to general triangulated domains, not only local stars\tcf{\sout{, though the latter are our main interest here}}. 
...
While the resulting potential operators feature generally lower regularity, they are conductive for our purposes. 
...
We also address the construction of potential operators for the gradient, curl, divergence, and more generally the exterior derivative over a simplex subject to partial boundary conditions. 
...
In light of the different approaches \tcf{d}iscussed above, we seek constants that can be explicitly computed in terms of the mesh geometry, that do not require solving global finite element problems, and that are independent of the boundary regularity of the domain.
...
We generalize this sequential construction of potentials to the curl, the divergence, and \tcf{more generally,} the exterior derivative. 
...
we \tcf{require} %% ensure
that the new simplex intersects with the existing subtriangulation along an $(n-1)$-dimensional boundary submanifold.
...
% 6. Contractible 
Inspired by the contractibility of shellable triangulations, we \tcf{pursue} %% pursuit
an additional estimate for Poincar\'e--Friedrichs constants. 
...
We \tcf{recap} %% move on to the
Sobolev spaces in vector calculus and the calculus of differential forms in Section~\ref{section:calculus}. 
...
\tcfsays{Should we be worried that $\underlying{\calT}$ is a closed set when working
with open domains?}
...
\tcfsays{Should this be the interior of the underyling set?}.
A crucial structural observation is the following.

\begin{lemma}
 Let $\calT$ be an $n$-dimensional simplicial complex and let $S,S' \in \calT$.
 Then either $\patch_{\calT}(S)$ and $\patch_{\calT}(S')$ are essentially disjoint \tcfsays{We should probably define presicely what this is.} or there exists $S'' \in \calT$
 such that 
 \begin{align*}
...
\begin{remark}
    The ratio $\aspectratio(T)$ measures the ``shape quality'' of an $n$-dimensional simplex $T$ and is \tcf{an} instance of a so-called \emph{shape measure}.
...
\tcfsays{I have removed the the subscripts from the $\|{\cdot}\|_2$ norm below, since the notation $\|{\cdot}\| = \|{\cdot}\|_2$ was introduced above.}

\tcfsays{I have changed the macros for {\tt \textbackslash Ceins\{n\}} and {\tt \textbackslash Czwei\{n\}} to immedtiatly print $\sqrt{n}$.}
...
\begin{lemma}\label{lemma:measurerelationships}
    ...
    \begin{gather*} % First pair 
        ...
        \tcf{\leq \Czwei{n} \cdot \height(T)^{-1}}
        \leq
        \Czwei{n} \cdot \aspectratio(T) \cred{\diam}(T)^{-1}
        ,
        \\
...
%        \diam(T) \frac{ n! \vol(T) }{ \diam(T)^{n} } \leq h_S \leq \diam(F).
	...
	\tcfsays{What are $z_1$ and $z_2$? I can imagine this are the vertices of $T_1$ and $T_2$ not contained in $F$, but this should be written.
             What is the Xu-Zikanato estimates?}
...
\end{comment}
    ...
    We extend $u$ by zero to a function over the entire $\bbR^{n}$ \tcfsays{How is that done? $u$ does not vanish on the whole boundary.}.
...
\tcfsays{Is the definition of trace satisfactory for matching traces across simplicies?}
...
\tcfsays{It is a bit pedantic, but shouldn't we say that $u$ is extended by zero to define the convolution below?}
...
\tcf{In the remainder of this section,} $\Omega \subseteq \bbR^{n}$ is a bounded convex open set with diameter $\diam(\Omega) > 0$.
...
We begin \tcf{by} introducing the Costabel-McIntosh kernel.
...
\tcfsays{I guess the way we remove the $\chi_\Omega$ is beause we somehow have a convex (if $r \leq \delta(\Omega)$) conbination
...
We are now \tcf{ready to} state the main results of this section. 
...
However, \tcf{\sout{though}} the space dimension $n$ and the form degree $\mia$ enter the estimates, 
...
\begin{theorem}\label{theorem:bogovpoinc}
    ...
    \begin{align}
        ...
        \\
        u(x) &= \cartan \Poinc_{\mia} u(x) + \Poinc_{\mia+1} \cartan u(x), \qquad 1 \leq \mia \leq n\tcf{,}
        \label{math:theorem:bogovpoinc:poincare:k}
    \end{align}
    \tcf{for a.e. $x \in \Omega$.} In particular, if $u \in W^{p}    \Alt^{\mia}(\Omega)$ with $0 \leq \mia \leq n-1$, then $w = \Poinc_{\mia} \cartan u$ satisfies $\cartan w = \cartan u$.
    ...
    \begin{align}
        ...
        \\
        u(x) &= \cartan \Bogov_{n} u(x) - \left( \vol(\Omega)^{-1} \int_{\Omega} u(a) \right) \chi_{\Omega}\tcf{,}
        \label{math:theorem:bogovpoinc:bogov:n}
    \end{align}
    \tcf{for a.e. $x \in \Omega$.} In particular, if $u \in W^{p}_{0}\Alt^{\mia}(\Omega)$ with $0 \leq \mia \leq n-1$, then $w = \Bogov_{\mia} \cartan u$ satisfies $\cartan w = \cartan u$.
...
\begin{proof}
    ...
    \tcf{\sout{We write $\zeta := \Poinc_{\mia  } u$.}}
    ...
    \begin{align*}
        ...
        .
    \end{align*}
    \tcf{Hence b}y definition, $\tcf{\Poinc_{\mia} u} \in W^{p}\Alt^{\mia-1}(\Omega)$ with $\cartan \tcf{\Poinc_{\mia} u} = u - \Poinc_{\mia+1} \cartan u$.
    ...
    \begin{align*}
        ...
        .
    \end{align*}
    By definition, $\tcf{\Bogov_{\mia} u} \in W^{p}_{0}\Alt^{\mia-1}(\Omega)$ with $\cartan \tcf{\Bogov_{\mia} u} = u - \Bogov_{\mia+1} \cartan u$.
...
We are interested in simplicial complexes that triangulate\tcf{\sout{d}} domains and which are \emph{shellable}.
...
We have seen that the boundary simplices of a manifold triangulation constitute a triangulation of the manifold\tcf{'}s boundary. 
...
\begin{lemma}\label{lemma:startopology}
    ...
    \begin{itemize}
        ...
        and $\carapace_{\calT}(S)$ is a simplicial $(n-1)$-\tcf{\sout{ball}sphere?}.
    \end{itemize}
\end{lemma}
\begin{proof}
    ...
    by the topological Schoenflies theorem \tcfsays{Reference here?}.
...
\begin{proof}
	...
	\tcfsays{I could not really follow the paragraph above. Let's talk about it when we can!}

    If the underlying space $\underlying{\calT}$ is connected, 
    then we easily see that the union of the $1$-simplices is path-connected. \tcfsays{Why?}
    Given $n$-simplices $S,S' \in \calT$, 
    we can thus choose a sequence of $\tcf{S = \hat S_0},\hat S_1,\dots,\hat S_m=S' \in \calT$ such that $\hat S_{i} \cap \hat S_{i-1} \neq \emptyset$ for all $1 \leq i \leq m$, and so each two consecutive simplices in that sequence have at least one vertex in common. 
...
\begin{remark}
    ...
    When the dimension is at least five, then there is no computer algorithm that, given a finite simplicial complex as input, \tcf{can} decide\tcf{\sout{s}} whether the input is the triangulation of some fixed manifold~\cite{chernavsky2006unrecognizability}.
	...
	\tcfsays{To make sure I properly understood: In the former case, you say that given a triangulation and a manifold, you cannot decides whether it triangulates it. In the second case,
	you say that it is not possible to decide whether a triangulation triangulates some manifold (without specifying what it is, neither as input nor output). Correct?}
...
\begin{proof}  
    ...
    which requires $m = M$. \tcfsays{I don't understand why $X_M$ couldn't be a topological $n$-ball.}
    On the other hand, 
...
\begin{proof}
    ...
    Any shelling of \tcf{$\calT$} can be extended to a shelling of $\calS$ by re-inserting the first triangle $S$.
...
\tcfsays{If the mappings are piecewise affine below, I guess that their actual norms
could be exactly computed rather than estimated as below. It might reduce the resulting
PF constants.}
...
\begin{proposition}\label{proposition:starreflection}
    ...
    \end{align*}
    which is the identity along \tcf{$\Gamma_1$ [--].} %${\Gamma_1} = T \cap \Makoto$. 
...
%     \end{gather*}
    \end{enumerate}
\tcfsays{Should $\sigma_n$ be $\sigma_n^{-1}$ in the second line of both statements?}
\end{proposition}
\begin{proof}
    ...
    \begin{itemize}
        ...
        We fix the subsimplex $S' \subseteq T$ that is complementary \tcfsays{What does that mean?} to $S$.
		...
		\tcfsays{A picture would be very nice to visualize what's going on here.}
        ...
        We \tcf{let} $h_{z} \in \bbR^{n}$ be the height vector of $z_{S'}$ inside the simplex $K$.
        We can write
        \begin{align*}
            \Theta_{|K}(x) 
            = 
...
\begin{lemma}\label{lemma:mixedbconsimplex:exteriorderivative}
    ...
    \begin{align*}
        ...
        2^{\frac{n-\ell}{p} + n + 1}
        %2^{n+1} 
        n^{\tcf{3/2}} C_{\Bogov}(n,k+1) \frac{ \vol_{}(B_1(0)) }{ (\ell+1)/n! } 
...
\tcf
{
\begin{lemma}\label{lemma:mixedbconsimplex:exteriorderivative:p2n3}
We make the same assumption as in Lemma~\ref{lemma:mixedbconsimplex:exteriorderivative},
but additionally require that $p=2$ and $d=3$. Then, for all 
$u \in W^{p}\Lambda^{k}(T)$ with $\trace_{T,\Gamma} u = 0$,
there exists $w \in W^{p}\Lambda^{k}(T)$ such that $\trace_{T,\Gamma} w = 0$,
$\cartan w = \cartan u$, and  
\begin{align*}
\| w \|_{L^{p}(\Omega)} 
\leq 
\dots
...
\tcfsays{I am currently working on the proof.}

%%%   \begin{proof}
%%%   We present in detail the case where $k=2$ and $\cartan = \curl$.
%%%   We set
%%%   \begin{equation*}
%%%   w
%%%   :=
%%%   \arg \min_{\substack{w \in W^{p}\Lambda^{k}(T) \\ \trace_{T,\Gamma} w = 0}} \|\phi\|.
%%%   \end{equation*}
%%%   
%%%   Here, we first assume that $T$ is the reference tetrahedron.
%%%   We observe that $w \in H^1(T)$. To see that, observe that $w$ solves a boundary
%%%   value problem in $T$. We can classically reflect $w$ into $\widetilde w$ across all
%%%   the faces of $T$ that are aligned with the coordinate axis (the reflection is ``odd''
%%%   or ``even'' depending on the boundary condition of the face, and this process is done
%%%   iteratively). The resulting $\widetilde w$ satifies $\widetilde w|_T = w$ and solves the
%%%   same PDE than $w$ in the reflected patch $D$ (it is a convex ``diamond'') with the boundary
%%%   condition satisfied by $w$ onto the face not aligned with the coordinate axis on the
%%%   whole boundary of the resulting domain. In other words, $\widetilde w$ solves a
%%%   curl-div PDE on a convex domain with one set of boundary conditions only, and therefore,
%%%   $\widetilde w \in H^1(D)$, so that in particular $w \in H^1(T)$.
%%%   \end{proof}
}
...
\begin{align*}
    ...
    .
\end{align*}
\tcfsays{I don't really understand what $\Gamma$ is in the formula above.}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        . 
    \end{align*}
	\tcfsays{As pointed out above, I don't think this makes sense for our definition of trace.}
...
\tcfsays{I have removed theorem specific to curl. I feel like the ones above stated with
the exterior derivatives are sufficiently clear.}
...
The most important outcome of this manuscript is the computation of upper bounds of Poincar\'e--Friedrichs constants for the curl and divergence operators over local patches in low \tcf{(i.e. $n=2$ or $3$)} dimensions. 
...
\tcfsays{We might want to change above paragraph.}
