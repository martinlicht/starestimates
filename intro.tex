...
% we then construct a potential over a neighboring simplex that shares at least one face with the one of the preceding simplices. 
% Of course, the potential on the new simplex and the existing intermediate potential may have different tangential traces along the interface between the new simplex and the existing intermediate subtriangulation.
...
% We start by constructing, say, a curl potential, over a single simplex. 
% Having already defined a potential operator over a subtriangulation, 
% we then construct a potential over a neighboring simplex that shares a face with the one of the preceding simplices. 
