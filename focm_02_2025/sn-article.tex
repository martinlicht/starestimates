...
\title[Computable PF over shellable triangulations]{Computable Poincar\'e--Friedrichs constants for the $L^{p}$ de~Rham complex over convex domains and domains with shellable triangulations}
...
\author[1]{\fnm{Th\'eophile} \sur{Chaumont-Frelet}}\email{theophile.chaumont@inria.fr}
\equalcont{These authors contributed equally to this work.}

\author*[2]{\fnm{Martin Werner} \sur{Licht}}\email{martin.licht@epfl.ch}

\author[3,4]{\fnm{Martin} \sur{Vohral\'ik}}\email{martin.vohralik@inria.fr}
\equalcont{These authors contributed equally to this work.}


\affil[1]{\orgname{Inria Univ. Lille and Laboratoire Paul Painlev\'e}, \orgaddress{\city{Villeneuve-d'Ascq}, \postcode{59655}, \country{France}}}

\affil*[2]{\orgname{\'Ecole Polytechnique F\'ed\'erale de Lausanne}, \orgaddress{\city{Lausanne}, \postcode{CH-1015}, \state{VD}, \country{Switzerland}}}

\affil[3]{\orgname{Inria}, \orgaddress{\street{48 rue Barrault}, \city{Paris}, \postcode{75647}, \state{State}, \country{France}}}
\affil[4]{\orgname{CERMICS, Ecole des Ponts}, \orgaddress{\city{Marne-la-Vall\'ee}, \postcode{77455}, \state{State}, \country{France}}}
...
\abstract{We construct potentials for the gradient, the curl, and the divergence operators over domains with shellable triangulations.
...
\keywords{Bogovski\u{\i}--Poincar\'e potential operator, Hodge--Laplace eigenvalues, Poincar\'e--Friedrichs constants, shellable triangulations, }

%%\pacs[JEL Classification]{D8, H51}

\pacs[MSC Classification]{58A10, 35A23, 65N30, 57Q99, 57Q05, 05E45}
...
For the purpose of illustration, we let $\Omega \subseteq \bbR^3$ be a bounded connected open set.
...
\begin{align}\label{math:intro:poincarefriedrichs:grad}
    ...
    .\tag{PF} 
\end{align}
Such an inequality holds if we can bound the \emph{operator norm} of this gradient potential operator. Then the best constant is
...
\end{align}
Finding the norm-minimizing potential and the optimal Poincar\'e--Friedrichs constant is generally tricky.
...
In the Banach space case, not even the existence of norm-minimizing potentials is trivial.
...
not much research seems available on computable constants in such Poincar\'e--Friedrichs inequalities for the curl operators.
...
For every $u \in W^{p}\Alt^{k}(\Omega)$, there exists $w \in W^{p}\Alt^{k}(\Omega)$ with the sam exterior derivative $\cartan w = \cartan u$ and such that\footnote{All the notation is fixed in detail in Section~\ref{section:calculus} later.}
...
% curl 
The core challenges are found in the discussion of the curl operator in three dimensions,
where Poincar\'e--Friedrichs inequalities have appeared under different names.
Let us assume momentarily that $\Omega$ is a weakly Lipschitz domain with trivial topology and consider only the Hilbert case $p=2$.
...
Guerini and Savo~\cite{guerini2004eigenvalue} address the spectrum of the Hodge--Laplace operator on bounded convex domains with smooth boundary in the Hilbert space case $p=2$. Among their results is the observation that the Poincar\'e--Friedrichs constant for the gradient already estimates the corresponding constants for the curl and divergence operators.
...
When the domain is not necessarily convex but star-shaped with respect to a ball,
...
If we conceive these potentials as mappings between Lebesgue spaces of differential forms, then their operator norms are upper estimates for the Poincar\'e--Friedrichs constants of the domain.
...
the estimates that rely on this geometric condition deteriorate when the aforementioned ball has a radius much smaller than the domain diameter.
...
The main objective of this manuscript is the construction of potentials for the gradient, curl, and divergence, as well as, more generally, the exterior derivative.
...
We also address the construction of potentials for the gradient, curl, and divergence operators, being special cases of the exterior derivative, over a simplex subject to partial boundary conditions.
...
Our main objective remains to find potentials for the differential operators of vector calculus and exterior calculus, including the curl and divergence over triangulated domains.
...
we require the new simplex to intersect with the existing subtriangulation along a boundary submanifold of dimension $n-1$, allowing us to define a well-posed auxiliary problem and to extend the existing curl potential to the new simplex.
...
where we also discuss the difference with the Poincar\'e inequality and motivate our interest in linear potentials.
We review basic notions of triangulations in Section~\ref{section:triangulations}.
Subsequently, we develop the computable upper bounds for the Poincar\'e--Friedrichs constants for the gradient over face-connected triangulated domains in Section~\ref{section:gradient}.
Sobolev spaces in vector calculus and the calculus of differential forms are reviewed in Section~\ref{section:calculus}. 
We then introduce our regularized potentials over convex sets in Section~\ref{section:potentialoperator}, 
...
Finally, we provide computable upper bounds for Poincar\'e--Friedrichs constants for the exterior derivative over shellable triangulations in Section~\ref{section:poincarefriedrichs} and present numerical examples in Section~\ref{section:numericalexamples}.
We conclude with an outlook in Section~\ref{section:outlook}. 
% LTeX: enabled=true
...
This section surveys variations of the Poincar\'e--Friedrichs inequalities for the gradient operator, with emphasis on analytical upper bounds over bounded convex domains.
...
\begin{remark}
    ...
    We highlight this perspective because it is our theoretical backbone when generalizing the discussion to the curl and divergence operators.
\end{remark}
...
(i) $\calT$ contains all the subsimplices of its members (ii) any non-empty intersection of two members of $\calT$ is a common subsimplex of each.
...
\begin{remark}\label{remark:mixedbconsimplex:hilbert}
    ...
    The variational formulation of the Poincar\'e constant over a convex domain reveals that $C_{\PF,T,F,p}$ lies between the Poincar\'e constant without boundary conditions and with full boundary conditions. 
    In particular,
    \begin{align*}
        C_{\PF,T,F,2} 
        \leq 
...
% DONE {TCF: Is the definition of trace satisfactory for matching traces across simplices?}
...
% DONE {I guess the way we remove the $\chi_\Omega$ is because we somehow have a convex (if $r \leq \delta(\Omega)$) combination of $x,y$, correct? It would be nice pointing it out explicitly.}
...
\end{theorem}
\begin{proof}    
    %     The discussion is similar as in the proof of Theorem~\ref{theorem:fullrecursivesum:grad}.
    %     this time geared towards the application of Theorem~\ref{theorem:poincarefriedrichsestimate:exterior}.
...
Theorem~\ref{theorem:poincarefriedrichsestimate:exterior} estimates the gradient, curl and divergence Poincar\'e--Friedrichs constants.
In addition, Theorem~\ref{theorem:poincarefriedrichsestimate:grad} provides another upper bound for the Poincar\'e constant.
...
The reference Poincar\'e constants are the inverse square roots of reference eigenvalues, the latter computed as solutions of finite element eigenvalue problems in mixed formulation over sufficiently refined meshes.

Computing the upper bound for the Poincar\'e constant of Theorem~\ref{theorem:fullrecursivesum:grad} requires computing paths between simplices.
One quickly sees that a spanning tree of the face-connection graph provides paths that minimize the estimate.
We choose a depth-first search that attempts to minimize the resulting constant.
While this does not take much time for small two-dimensional examples, the computation takes considerably longer for our three-dimensional examples.

We require a shelling to apply Theorem~\ref{theorem:fullrecursivesum:grad} and estimate the upper bound for the Poincar\'e--Friedrichs constant.
...
While a brute-force enumeration of shellings is still feasible for small two-dimensional triangulations, it becomes practically infeasible in dimension three due to the combinatorial explosion of the number of possible shellings.
Instead, we perform a backtracking search guided by a greedy strategy:
Starting with a single simplex, we successively add simplices to the partial shellings until a complete shelling is found.
%
%
In choosing among the possible candidates for the next simplex, our practical heuristic prefers those that introduce the smallest possible geometric constants, and we further prioritize the completion of face stars because then the induced constants are easier to control via Lemma~\ref{lemma:volumecomparison}.
%
%
We repeat this backtracking approach for all possible initial simplices, producing up to $10$ shellings each time, and pick the optimal shelling among those found.
We have observed in our computations that the Poincar\'e--Friedrichs estimates depend on the shelling and may differ by orders of magnitudes;
the method might not necessarily produce the global optimum.

In order to ensure tight estimates, we compute the relevant geometric parameters, such as heights, individually for each simplex and use individualized estimates for each local star. These estimates are numerically tighter than the ones stated in Theorems~\ref{theorem:poincarefriedrichsestimate:grad} and~\ref{theorem:poincarefriedrichsestimate:exterior}, as we directly implement the calculations used in the proofs.


% and pick the optimal shelling among those found.
...
% Therefore, we need a practical heuristic to find shellings that induce small constants.
...
% In other words, we defer the completion of non-face stars to postpone large factors to the end of the recursion.
...
because that constant appears repeatedly within our main result.
...
With an overestimate by a factor of at most six,
the estimate seems within practical range, at least on simplices with good shape regularity. 
However, the results indicate room for improvement, and we expect the overestimate to worsen as the eccentricity of the tetrahedron increases.
Future estimates should tighten the gap towards the practically observed values, including for ill-shaped simplices.
...
% (with Remark~\ref{remark:improved_starreflection}).
...
\begin{table}[t]  
    ...
    that rely on Theorem~\ref{theorem:poincarefriedrichsestimate:exterior} with $k=1$ (3rd and 4th column) and with $k=2$ (5th and 6th column).
    Note that the divergence constant can always be estimated using Lemma~\ref{lemma:mixedbconsimplex}.
    ...
    \begin{tabular}{|r||r|r|r||r|r|r|}
        ...
        \hline
        $\calT_{C,5}$   & 0.225        & 141.148     & 627.310                & 0.183             & 3.391             & 18.453          \\
        \hline
        $\calT_{C,K}$   & 0.225        & 12.157      & 54.030                 & 0.183             & 25.899            & 140.920         \\
        \hline
        $\calT_{B,5}$   & 0.331        & 113.084     & 341.056                & 0.233             & 152.886           & 655.993         \\
        \hline
        $\calT_{B,K}$   & 0.331        & 162.687     & 490.655                & 0.233             & 512.273           & 2198.026        \\
        \hline
        $\calT_{F}$     & 0.554        & 25752.342   & 46403.302              & 0.310             & 8958.467          & 28835.266       \\ 
        \hline
    \end{tabular}
\end{table}
...
However, we believe there are ample opportunities to refine our estimates of Poincar\'e--Friedrichs constants, both conceptually and algorithmically, and extend the technique to further applications.
...
On the algorithmic part, even though merely checking whether a general triangulation has a shelling is known to be computationally infeasible \cite{goaoc2019shellability},
