...
% TODO {[TCF: I believe this is often refered to as the Tartare-Petree theorem]}
...
% TODO {[TCF: Could you reference a particular theorem or chapter?]} 
...
% TODO {Replace the constant Ceins and Czwei by the square root of n immediately.}
...
\begin{proof}  
    ...
    which requires $m = M$.
    % TODO {TCF: I don't understand why XM couldn't be a topological n-ball.}
    On the other hand, 
...
\begin{proposition}\label{proposition:starreflection}
    ...
    \todo{Include a picture of the situation?}
    At any point, the singular values $\sigma_1 \geq \dots \geq \sigma_n$ of its Jacobian satisfy 
    \begin{gather*}
...
%         \end{align*}
        ...
        %         In oder to estimate $\vecnorm{ h_{G} }$, recall that $x = \lambda (-z_{S'}) + (1-\lambda) z_{x}$ for some $\lambda \in [0,1]$ and $z_{x} \in G$
