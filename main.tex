...
These quantify the fundamental stability properties of \todo{numerous} partial differential equations
and enter the stability and convergence theory of numerical methods. 
Upper bounds of the Poincar\'e--Friedrichs constants also provide lower bounds of the eigenvalues of the associated Laplacians \todo{and the eigenvalues of finite element matrices. (doubtful statement?)}
...
for example, local triangulations around a simplex \todo{[TCF: Is it really
true if the simlex is of highest dimension, e.g., a tetrahedron in 3D?]} within a larger triangulation (the so-called local patches or stars) are shellable in dimensions two and three. 
...
Their inequalities hold in convex Lipschitz domains, too, still in the Hilbert space setting~\cite{licht2024geometry}.
However, no results as those of~\cite{guerini2004eigenvalue} are known over bounded convex Lipschitz domains and for general Lebesgue exponents $1 \leq p \leq \infty$. %\todo{ [TCF: We might want to obtain with ML's recent preprint.].}
...
In fact, Poincar\'e--Friedrichs inequalities even hold over any star-shaped open bounded set~\cite[Theorem~3.1]{hurri1988poincare}. \todo{[TCF: I don’t exactly get the point of this sentence.].}
...
% TODO: Figure out whether to use the following paragraph.
...
% TODO: check whether introduction is readable
...
%         \leq  
%         \| b - b' \| + | \alpha - \alpha' | \aspectratio(T_1) \frac{\diam(T_2)}{\diam(T_1)} \| \hat h_0 \|
%         \leq 
...
\todo{TCF: It is a bit pedantic, but shouldn’t we say that u is extended by zero to define the convolution
below?}
...
\begin{proof}
    ...
    \todo{Get rid of the auxiliary zeta?}
    
    
    Consider now $u \in W^{p}\Alt^{\mia}(\Omega)$ with $1 \leq k \leq n$. 
    \todo{\sout{We write $\zeta := \Poinc_{\mia  } u$.}}
...
\begin{lemma}\label{lemma:startopology}
    ...
    \begin{itemize}
        ...
        and $\carapace_{\calT}(S)$ is a simplicial $(n-1)$-ball. \todo{sphere?}
    \end{itemize}
\end{lemma}
\begin{proof}
...
\begin{proof}
    ...
    \todo{TCF: I could not really follow the paragraph above. Let's talk about it when we can!}
    
    If the underlying space $\underlying{\calT}$ is connected, 
    then we easily see that the union of the $1$-simplices is path-connected. \todo{Maybe not that easy.}
    Given $n$-simplices $S,S' \in \calT$, 
    we can thus choose a sequence of $S = \hat S_0,\hat S_1,\dots,\hat S_m=S' \in \calT$ such that $\hat S_{i} \cap \hat S_{i-1} \neq \emptyset$ for all $1 \leq i \leq m$, and so each two consecutive simplices in that sequence have at least one vertex in common. 
...
\begin{lemma}\label{lemma:stardivision}
    Let $T$ be an $n$-dimensional simplex with a $\ell$-dimensional subsimplex $S$ and let $z_S$ be the barycenter of $S$. 
...
\begin{proposition}\label{proposition:starreflection}
    ...
    \end{align*}
    which is the identity along $\partial \Makoto \setminus \partial T$. \todo{Use some name for this intersection?}
    At any point, the singular values $\sigma_1 \geq \dots \geq \sigma_n$ of its Jacobian satisfy 
    \begin{gather*}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        We fix the subsimplex $S' \subseteq T$ that is complementary to $S$ within the simplex $T$. \todo{What is the complementary subsimplex?}
...
%             \frac{ \diam(T)^{n} }{ \vol(T) }
        ...
        Notice that 
        \begin{align*}
            \det \Jacobian\Phi = \sigma_{\max}(\Phi,G) \sigma_{\min}(\Phi,G) = \frac{\rho(1+\rho)}{(1+\rho)^2}.
        \end{align*}
        We now recall that the height of $h_{z}$ in $G \in \calG$ equals $(\ell+1)^{-1}$ multiplied with the height of some vertex of $S$ within $T$.
        Similar as above, we use the upper bound 
        \begin{align*}
...
\begin{proof}
    ...
    We write $\ell$ for the dimension of $S_{m}$.
