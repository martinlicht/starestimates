...
% DONE \todo{Theo : I believe Gallistl-Olkhovskiy also estimate the constant. They do star-shape and regularized stuff, so it's different, but we should keep that in mind and maybe reference them}.
...
\begin{proof}
    ...
    Lastly, letting $\Xi : T_1 \rightarrow T_2$ be as above, Lemma~\ref{lemma:affinetransform} and the first estimate imply 
    \begin{align*}
        \| \Jacobian \Xi \|
        &
        \leq 
    ...
    are all one with the possibe exception of the singular values of the matrix 
    \begin{align*}
        A = 
        \begin{pmatrix}
            a_1/h & 0
