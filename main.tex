...
then a Poincar\'e inequality is already implicit in polynomial interpolation estimates that are known to the mathematical community~\cite{...}. 
...
We are aware of work by Guerini and Savo who estimate the Poincar\'e--Friedrichs constants over convex domains in the Hilbert space setting, when $p=2$ and no boundary conditions are imposed. 
In the Hilbert space setting when $p = 2$, Guerini and Savo~\cite{guerini2004eigenvalue} have develop lower bounds for the spectrum of the Hodge--Laplace operator over convex domains. This implies upper bounds for the Poincar\'e--Friedrichs constant of the exterior derivative, and hence for the curl and divergence operators.
...
we are not aware of computable estimates for the case of $\bfH(\curl)$ and $\bfH(\divergence)$. % TODO
Clearly, not all local patches describe convex subdomains. 
One possible avenue towards Poincar\'e--Friedrichs constants is the observation that stars in triangulations are often star-shaped with respect to a ball. 
...
Veeser and Verfürth have provided computable upper bounds in the case of the classical Sobolev space $W^{1,p}(\Omega)$ over finite element stars~\cite{veeser2000}. 

% TODO:V&V seems to be the only reference for stars .... 
General results of this type for a class containing the finite element stars are not known to us. 
In particular and as will be illustrated in a moment, 
Poincaré constants of stars and thus of star-shaped domains may be arbitrarily large. 
Consequently, corresponding bounds have to incorporate some quantity that is sensible to the possible lack of
convexity.


There is a popular avenue towards estimates that is able to circumvent the effect of low boundary regularity. 
...
The regularized Bogovskii potential operator was studied extensively by Costabel and McIntosh.
These operators are defiend over sets star-shaeped with respect to a ball. 
...
While the resulting Bogovskii-type potential has more moderate regularity properties, 
...
\color{red}Our upper bounds do not depend on the angles of such re-entrant corners, even in the aforementioned limit cases, but only the shape-regularity of the triangulation. \color{black}
\\
...
\begin{align}
    ...
    \\
    v_0 + \sum_{i=1}^{k} \lambda_{i} ( v_i - v_0 ) + \sum_{i=k+1}^{n} \lambda_{i} ( v_n - v_i ),
\end{align}
\begin{align}
