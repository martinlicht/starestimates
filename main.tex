...
%                 \draw (O) -- (E);
    ...
    $\Gamma_2$ is left invariant by the deformation.}\label{figure:geometricconstruction}
\end{figure}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        % TODO: The above argument seems to rely on convexity. Generalise it to avoid that difficulty.
...
We compare computed Poincar\'e--Friedrichs constants over the tetrahedron with tangential boundary conditions along a few faces. 
...
A brufe-force search easily enumerates all the shortest face-paths that start from a given triangle, so we use the best possible estimate that this method provides. 

We compare the reference Neumann and Dirichlet eigenvalues with the bounds in Theorem~\ref{theorem:poincarefriedrichsestimate:exterior} and Theorem~\ref{theorem:poincarefriedrichsestimate:exterior:contraction}.
...
so the best possible estimate with that method can be obtained. 
...
The reference Poincar\'e--Friedrichs constants for the gradient, curl, and divergence operators are found via standard finite element eigenvalue computations, using the lowest-order finite element de~Rham complex and a sufficiently refined mesh (four steps of uniform refinement).
...
This greedy backtracking algorithm does generally not guarantee that the optimal shelling is found before the last iteration. 
Our implementation starts with a fixed simplex and generates up to $10$ shellings. 
We repeat this for a possible choices of initial simplex, and pick the optimal among those.
