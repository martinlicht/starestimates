...
% \todo{[TCF: Is it really true if the simlex is of highest dimension, e.g., a tetrahedron in 3D?]} 
...
% \todo{[TCF: I don’t exactly get the point of this sentence.].}
...
\begin{tabular}{ccc}
  \begin{tikzpicture}
    ...
    % TODO: Change colors to grey scale 
  ...
  \end{tikzpicture}
\end{tabular} 
\caption{From left to right: local patches around a vertex, and edge, and a triangle. The local patch of a full-dimensional simplex is the only the simplex itself (and its subsimplices).}\label{figure:patches}
\end{figure}
...
% \todo{TCF: It is a bit pedantic, but shouldn’t we say that u is extended by zero to define the convolution below?}
...
\begin{proof}
    ...
    % DONE \todo{Get rid of the auxiliary zeta?} {no}
...
\begin{proof}
    ...
    Given \(n\)-simplices \(S, S' \in \calT\), 
    there exists a path \(\gamma : [0,1] \rightarrow \underlying{\calT}\) from the barycenter of \(S\) to the barycenter of \(S'\).
    We can choose a sequence of \(n\)-simplices \(\hat S_0, \hat S_1, \dots, \hat S_m \in \calT\) (with \(\hat S_0 = S\) and \(\hat S_m = S'\)), without repetitions, 
    that covers the path and whose intersections with the path are homeomorphic to \([0,1]\).
    The sets \(\gamma([0,1]) \cap \hat S_{i}\) and \(\gamma([0,1]) \cap \hat S_{i+1}\) intersect at exactly one point, which lies in a common subsimplex of \(\hat S_{i}\) and \(\hat S_{i+1}\).
    We deduce that any two consecutive simplices in the sequence \(\hat S_0, \hat S_1, \dots, \hat S_m\) share at least one vertex. 
    As each vertex star is face-connected, 
    there exists another sequence of \(n\)-simplices \(T_0, T_1, \dots, T_m \in \calT\) (with \(T_0 = S\) and \(T_m = S'\)) 
    such that \(T_{i} \cap T_{i-1}\) is a face of both \(T_{i}\) and \(T_{i-1}\) for all \(1 \le i \le m\).
    This shows that \(\calT\) is face-connected.
    }
