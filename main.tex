...
% DONE \todo{Reformulate the first paragraph. } % 
...
We wish to clarify the relationship between the Poincar\'e--Friedrichs inequality, in the sense introduced above, with other inequalities that are known as Poincar\'e inequality (or also Poincar\'e--Wirtinger or Friedrichs inequality) in the literature. Given $p \in [1,\infty]$ and an open set $\Omega \subseteq \bbR^{n}$ of finite measure, % TODO: cite AAAA somebody, such as Alexandre 
...
\begin{theorem}\label{theorem:poincarefriedrichsestimate:grad}
    ...
    \begin{align*} % TODO: UNIFY NOTATION FOR THE CONSTANTS 
        % \color{red}
        \| w \|_{L^{p}(T_m)}
        &\leq 
...
\begin{proof}
    ...
    % TODO: cite E&V Math Comp 2020, Lemma B1. 
\end{proof}
...
Stars around subsimplices can be turned into vertex stars after a subdivision, 
and the shape regularity can be controlled. 
...
\begin{theorem}\label{theorem:starreflection}
    ...
    satisfying
    \begin{align*}
        \Lip( \varphi,      T ) \leq \Cvier{n}{k} \diam(T),
        \quad 
        \Lip( \varphi^{-1}, M ) \leq \Cfive{n}{k} \diam(T),
    \end{align*}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        The subsimplex $S' \subseteq T$ that is complementary to $S$.
        such that $J$ is the union of exactly those faces of $T$ that contain $S'$.
        ...
        We define a triangulation $\calS$ by the barycentric refinement of $T$. 
        All $n$-simplices in $\calS$ contain $S$ and $z'_{S}$.
        
        We define another simplicial complex $\calS^c$ as follows:
        whenever $K \in \calS$ is an $n$-simplex, we replace the vertex $z_{S}'$ by new vertex opposite point $-z_{S}'$,
        ...
        In what follows, we also use $\calS''$, the simplicial complex obtained from $\calS'$ by barycentric subdivision of $S$.
        Then $\calS''$ is the local star around the barycenter $z_S$. 
        
        \item 
        \todo{Reflect onto complement and estimate bi-Lipschitz constant}
        We introduce a new mapping $\Theta : |\calS| \rightarrow |\calS^c|$.
        
        
        \item 
        Consider any $n$-simplex $T$ with vertices $v_0, v_1, \dots, v_n$, where $v_0$ is the origin. 
        We let $A_T \in \bbR^{n \times n}$ be the matrix that maps the $i$-th unit vector to $v_i$.
        That matrix maps the reference simplex $\Delta^{n}$ onto $T$. 
        Then the mapping 
        \begin{align}
            F_T(x) 
            = 
            \frac{ \| A^{-1} x \|_{1} }{ \|x\|_2 } x
        \end{align}
        maps $T$ onto a ``simplex with a curved face'' inside the Euclidean unit ball. Its inverse is
        \begin{align}
            F_T^{-1}(x) 
            = 
            \frac{ \|x\|_2 }{ \| A^{-1} x \|_{1} } x
        \end{align}
        ...
        \begin{align}
            ...
            \left( \vecone \cdot A^{-1} x \right)
            + 
            \frac{x}{\|x\|} \left( \vecone \cdot A^{-1} x \right)
            .
        \end{align}
        ...
        \begin{align}
            \| \Jacobian F_T(x) \| 
            \leq 
            2 \left| \vecone \cdot A^{-1} x \right|
            \leq 
            2 \sqrt{n} \cdot \sigma_{\max}(A^{-1}) 
            .
        \end{align}
        ...
        \begin{align*}
            ...
            \leq 
            \left\| 
                \frac{ \| A^{-1} x \|_{1} }{ \|x\|_2 } x - \frac{ \| A^{-1} y \|_{1} }{ \|y\|_2 } y
            \right\|_{2}
            \\&\qquad 
            \leq 
            \left\| 
                \frac{ \| A^{-1} x \|_{1} }{ \|x\|_2 } x - \frac{ \| A^{-1} x \|_{1} }{ \|x\|_2 } y
            \right\|_{2}
            +
            \left\| 
                \frac{ \| A^{-1} x \|_{1} }{ \|x\|_2 } y - \frac{ \| A^{-1} y \|_{1} }{ \|y\|_2 } y
            \right\|_{2}
            \\&\qquad 
            \leq 
            \frac{ \| A^{-1} x \|_{1} }{ \|x\|_2 } 
            ...
            \right\|_{2}
            +
            \left| \|A^{-1} x\|_2 - \|A^{-1} y\|_2 \right|
            \\&\qquad 
            \leq 
            \left( \sqrt{n} + 1 \right) \sigma_{\max}(A^{-1}) 
            \left\| x - y \right\|_{2}
            .
        \end{align*}
        On the other hand, given $x, y \in F_T(T)$, 
        where $\| A^{-1} y \|_{1} \leq \| A^{-1} x \|_{1}$ without loss of generality, we find 
        \begin{align*}
            &
            ...
            \leq 
            \left\| 
                \frac{ \|x\|_2 }{ \| A^{-1} x \|_{1} } x - \frac{ \|y\|_2 }{ \| A^{-1} y \|_{1} } y
            \right\|_{2}
            \\&\qquad 
            \leq 
            \left\| 
                \frac{ \|x\|_2 }{ \| A^{-1} x \|_{1} } x - \frac{ \|x\|_2 }{ \| A^{-1} x \|_{1} } y
            \right\|_{2}
            +
            \left\| 
                \frac{ \|x\|_2 }{ \| A^{-1} x \|_{1} } y - \frac{ \|y\|_2 }{ \| A^{-1} y \|_{1} } y
            \right\|_{2}
            \\&\qquad 
            \leq 
            \frac{ \|x\|_2 }{ \| A^{-1} x \|_{1} } 
        ...
        We recall that $\sigma_{\max}(A^{-1}) = \sigma_{\min}(A)^{-1}$ and $\sigma_{\min}(A^{-1})^{-1} = \sigma_{\max}(A)$.
...
\[
    \Omega_{C}  = (-1,1)^2,
    \qquad 
    \Omega_{F}  = (-1,1)^2 \setminus (0,1)^2,
    \qquad 
    \Omega_{B} = (-1,1)^2 \setminus ( (0,1) \times \{0\} ),
\]
% Cube 
% Fichera
% Crossed bricks 
