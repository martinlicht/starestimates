...
\begin{theorem}\label{theorem:starreflection}
    ...
    satisfying
    \begin{gather*}
        \| \Jacobian\Xi \|          \leq \Cvier{n}(\calT),
        \quad 
        \| \Jacobian\Xi^{-1} \|     \leq \Cfive{n}(\calT),
        \\
        \| \det\Jacobian\Xi \|      \leq \Cdetvier{n}(\calT),
        \quad 
        \| \det\Jacobian\Xi^{-1} \| \leq \Cdetfive{n}(\calT),
    \end{gather*}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \end{align*}
        Let $\beta$ be the angle between $z_{S'}$ and $h_{z}$. 
        By the definition of the cosine, $\cos(\beta) \leq \| h_{z} \| / \| z_{S'} \|$. 
        Let $F_{z} \subseteq K$ be the face opposite to $z_{S'}$,
        which is also a face of $T$. Then $(n-k-1) h_{z}$ is the height vector of $F_{z}$ in $T$. 
        Thus 
        \begin{align*}
            \frac{ \| h_{z} \| }{ \| z_{S'} \| }
            \geq 
            \frac{ (n-k-1)^{-1} \| (n-k-1) h_{z} \| }{ \| z_{S'} \| }
            \geq 
            \frac{ (n-k-1)^{-1} n \vol(T) }{ \| z_{S'} \| \vol(F_{z}) }
            \geq 
            \frac{n}{ n-k-1 }
            \cdot 
            \frac{ \vol(T) }{ \diam(T)^{n} }
...
%             \\
%             \leq 
%             \frac{ n \vol(K) }{ \| z_{S'} \| \vol(F_{z}) }
%             \leq 
%             \frac{ \vol(T) }{ \| z_{S'} \| \vol(F_{z}) }
%             \leq 
%             \frac{ \vol(T) }{ \diam(T)^{n} }
%             .
        \end{align*}
...
What are the singular values of a $2x2$ (lower) unit triangular matrix? Suppose we have the matrix
...
\begin{align*}
    ...
    \end{pmatrix}
\end{align*}
Following up a generic formula found here, 
https://math.stackexchange.com/a/861774/3557
I decompose 
\begin{align*}
    \begin{pmatrix}
    1 & 0
    ...
    \end{pmatrix}
\end{align*}
The singular values $w_1$ and $w_2$ are implicitly given by 
\begin{align*}
    \frac{ w_1 + w_2 }{2} = \frac 1 2 \sqrt{ 4 + b^2 }
    \\
    \frac{ w_1 - w_2 }{2} = \frac 1 2 |b|
\end{align*}
If we assume that the first value is at least the second value, 
then adding and subtracting these equations leads to 
\begin{align*}
    w_1 = \frac 1 2 \sqrt{ 4 + b^2 } + \frac 1 2 |b|
    \\
    w_2 = \frac 1 2 \sqrt{ 4 + b^2 } - \frac 1 2 |b|
\end{align*}
I would like to know whether that reasoning is correct 
...
\end{align*}
        ...
        Its largest and the smallest singular values $\sigma_{max} \geq 1 \geq \sigma_{\min}$ are~\cite{blinn1996consider} 
        \begin{align*}
...
%         \end{align*}
        ...
        by scaling with $(k+1)^{-1}$. Each of the latter height vectors has length bounded from below by the height of some vertex in $S$ within a simplex of $\calS$, multiplied by $\kappa(A)^{-2}$. The height of any vertex of $S$ in some simplex of $\calS$ is the same as the height $h_T$ of that vertex in the original simplex $T$. 
        ...
        \begin{align*}
            \frac{ \|h_{F}\|}{\diam(T)}
            = 
            \frac{ n \vol(T) }{ \vol(F) \diam(T) }
            \geq 
            \frac{ n \vol(T) }{ \diam(T)^{n} }
            \geq 
            (n-1)! \cdot \frac{ \diam(T)^{n} }{ n! \vol(T) }
            .
        \end{align*}
        We conclude that 
        \begin{gather*}
            ...
            (n-1)! \cdot \frac{ \diam(K)^{n} }{ n! \vol(K) }
            .
        \end{gather*}
        \begin{gather*}
            \frac{ \diam(K) }{ \| h_{{K}} \| }
        ...
        \begin{align*}
            ...
            (k+1) 
            (n-1)! \cdot 
            \left( 1 + \kappa(A)^{2} \right)
            \sup_{ K \in \patch_{\calT}(S) }
            \frac{ \diam(K)^{n} }{ n! \vol(K) }
            .
        \end{align*}
        \todo{bound $\kappa(A)$ right away...}
