...
\subsection{Potentials and Poincar\'e--Friedrichs inequalities for the gradient}
...
\end{align}
In the present setting, where $\Omega$ is connected and the potential of the different gradient potentials can thus only differ by a constant, $\Phi_{\grad}( \bff )$ is a one-dimensional convex minimization problem. 
...
Attention has been given to \emph{linear} potential operators
...
Its operator norm is the constant $C_{\varnothing,\Omega,p} > 0$ in the \emph{Poincar\'e inequality} 
...
In particular, the Poincar\'e inequality and the Poincar\'e--Friedrichs inequality generally hold with different constants.  
Considerable research effort has gone into computing (or estimating) the optimal constants in the Poincar\'e--Friedrichs inequalities~\eqref{math:intro:poincarefriedrichs:grad} or Poincar\'e inequalities~\eqref{math:intro:poincare:grad}. Upper estimates for these constants also correspond to lower bounds for the spectra of Neumann--Laplacians over those domains. 
...
\subsection{Potentials and Poincar\'e--Friedrichs inequalities for the curl and divergence}
...
Our interest are linear potential operators 
\begin{align*}
...
\subsection{Potentials and Poincar\'e--Friedrichs inequalities for the exterior derivative}
...
We review the literature on Poincar\'e--Friedrichs inequalities. We also identify the obstructions of presently known results that we want to overcome with our contributions. 
...
Guerini and Savo~\cite{guerini2004eigenvalue} provide explicit lower bounds for the spectrum of the Hodge--Laplace operator on bounded convex domains with smooth boundary in the Hilbert space case $p=2$.
These bounds are only in terms of the dimension and diameter of the convex domain. 
They thus give upper estimates for the Poincar\'e--Friedrichs constant of the exterior derivative, and hence in particular for the curl and divergence operators.
Additionally, a comparison result shows that the Poincar\'e--Friedrichs constant for the gradient already estimates the corresponding constants for the curl and divergence operators. 
Using a duality argument, this also gives upper estimates of the Poincar\'e--Friedrichs constants for the gradient, curl, and divergence operators subject to Dirichlet, tangential, and normal boundary conditions, respectively, along the entire boundary. 
However, these upper bounds are not necessarily optimal. 
Moreover, no results as those of~\cite{guerini2004eigenvalue} are known over bounded convex Lipschitz domains and for general Lebesgue exponents $1 \leq p \leq \infty$. 
...
Recall that the main objective of this manuscript is bounding these constants over local stars within triangulated domains. 
Clearly, not all local stars describe convex subdomains, but most local stars are star-shaped with respect to a ball. 
This observation would enable, e.g., the averaged Poincar\'e and Bogovski\u{\i} operators~\cite{costabel2010bogovskiui}. 
...
Geometric settings that admit finite triangulations offer further pathways to obtain Poincar\'e--Friedrichs inequalities. 
...
Another class of upper bounds for Poincar\'e--Friedrichs inequalities uses some form of passing through the triangulation and constructs the potentials step-by-step. 
...
This sequential procedure applies to general triangulated domains, not only local stars, though the latter is our main interest here. 
Importantly, these sequential estimates of Poincar\'e--Friedrichs constants generally circumvent the effect of low boundary regularity and only rely on the shape regularity. 




\subsection{Review: General results}

Not much attention however seems to have been given to the study of computable constants in~\eqref{math:pot_PF_curl_div}, even if the domain $\Omega$ is convex. 
...
Let for a moment the topology of $\Omega$ be trivial, let $\Omega$ be weakly Lipschitz, and consider the Hilbert case $p=2$. 
Then, \eqref{math:pot_PF_curl} is related to the so-called Poincar\'e--Friedrichs--Weber inequality
\begin{align}
    \label{math:Weber}
...
\label{math:PF_orth} \begin{align}
	...
	\\
    \| \bfu\|_{L^{2}(\Omega)} & \leq \tilde C_{\divergence,\Omega,p} \| \divergence \bfu \|_{L^{2}(\Omega)}, 
...
We refer to Equation~(5) in~\cite{Fried_diff_forms_55}, Equation~(2) in Gaffney~\cite{Gaff_Hilbert_harm_55}, Weber~\cite{Web_compact_Maxw_80}, \cite[Lemmas~3.4 and~3.6]{Gir_Rav_NS_86}, \cite[Proposition~7.4]{Fer_Gil_Maxw_BC_97}, and the references therein. 
Still in the Hilbert space case $p=2$, the Friedrichs inequality~\cite{burenkov1998sobolev} over the Sobolev space with homogeneous Dirichlet boundary conditions on $\partial \Omega$ implies, via duality~\eqref{math:pot_PF_div}. However, that easy observation seems restricted to $p=2$. 
...
We rely on regularized potential operators as a remedy. 
...
\subsection{Objectives: Potentials subject to partial boundary conditions}
...
\subsection{Objective: Main results}
...
\subsection{Objective: Combinatorial constants over shellable triangulated domains}
