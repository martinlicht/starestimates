...
\begin{proposition}\label{proposition:starreflection}
    ...
    and let 
    \begin{align*}
        \Minako := \overline{ \underlying{\patch_{\calT}(S)} \setminus T },
        \qquad 
        {\Gamma_1} := \Minako \cap T,
        \qquad 
        {\Gamma_2} := \overline{ \partial T \setminus \partial \Minako }.
    \end{align*}
    ...
    \begin{align*}
        \Xi_{1} : T \rightarrow \Minako
    \end{align*}
    which is the identity along ${\Gamma_1} = T \cap \Minako$. 
...
%     \end{gather*}
    ...
    There exists a bi-Lipschitz piecewise affine mapping
    \begin{align*}
        \Xi_{2} : \underlying{\patch_{\calT}(S)} \rightarrow \Minako
    \end{align*}
    which is the identity along $\partial \Minako \setminus \partial T$.
    At any point, the singular values $\sigma_1 \geq \dots \geq \sigma_n$ of its Jacobian satisfy 
    \begin{gather*}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        % Let $I := \Minako \cap T$, which is the union of $k$ different faces of $T$, with $1 \leq k \leq n$.
        ...
        whereas ${\Gamma_1} = \Minako \cap T$ is the union of exactly those faces of $T$ that contain $S$. 
...
%         \end{align*}
        ...
        \item 
        We define a mapping $ \Xi_{1} : T \rightarrow \Minako = \overline{ \underlying{ \patch_{\calT}(S) } \setminus T}$ by setting 
        \begin{align*}
            \Xi_{1} := \Psi \circ \Theta.
        \end{align*}
        It preserves the interface from $T$ onto $\Minako$. 
        ...
        \item 
        We define another mapping $ \Xi_{2} : \underlying{\patch_{\calT}(S)} \rightarrow \Minako $ by setting 
        \begin{align*}
            \Xi_{2} := \Psi \circ \Phi.
        \end{align*}
...
\begin{proof}
    ...
    onto the face $F_i$. 
    In what follows, we let $\widetilde \Minako$ be the convex set obtained from reflecting $\Delta^n$ along the coordinate axes $l+1$ through $n$. 
    We see that $\vol(\widetilde \Minako) = (l+1)/n!$
    
    We let $\hat u := \varphi^{\ast} u$ and define $\hat g \in L^{p}(\Delta^n)$ via $\hat g := \cartan \varphi^{\ast} u$. 
    Then $\cartan \hat u = \hat g$. 
    We let $\tilde u$ be the extension of $\hat u$ onto $\widetilde \Minako$ by reflection across the coordinate axes,
    and let $\tilde g$ is the extension of $\hat g$ onto $\widetilde \Minako$ by reflection across the coordinate axes. 
    We observe $\tilde g = \cartan \tilde u$. 
    By construction, $\tilde u \in W^{p}_{0}\Lambda^{k}(\widetilde \Minako)$.
    
    The bounds for the Bogovski\u{i} potential operators, Theorem~\ref{theorem:bogovpoinc} imply 
    the existence of $\tilde w \in W^{p}_{0}\Lambda^{k}(\widetilde \Minako)$
    with $\cartan \tilde w = \cartan \tilde u$ and 
    \begin{align*}
        \| \tilde w \|_{L^{p}(\widetilde \Minako)}
        &\leq 
        n C_{\Bogov}(n,k+1) \frac{ \vol_{}(B_2(0)) }{ \vol(\widetilde \Minako) } 
        2
        \| \tilde g \|_{L^{p}(\widetilde \Minako)}
        \\&\leq 
...
\begin{align*}
    \Minako_{m-1} := \patch_{\calT}(S_{m}) \setminus T_{m}.
\end{align*}
for the complement $\Minako_{m-1} \subseteq X_{m-1}$ of the simplex $T_{m}$ in the star $\patch_{\calT}(S_{m})$. 
...
\begin{proof}
    ...
    We introduce the reflection mapping $\Xi_{1} : T_{m} \rightarrow \Minako_{m-1}$ of Proposition~\ref{proposition:starreflection}. 
    ...
    % \begin{align*}
        ...
        % \|\det \Jacobian \Xi_{1}^{-1}  \|_{L^{\infty}(\Minako_{m-1})}^{\frac 1 p} 
        % \cdot 
        ...
        % \|\det \Jacobian \Xi_{1}^{-1}  \|^{\frac 1 p}_{L^{\infty}(\Minako_{m-1})} 
        % \cdot 
...
\begin{proof}
	...
	We introduce the deformation mapping $\Xi_{2} : \patch_{\calT}(S_M) \rightarrow \Minako_{M-1}$ of Proposition~\ref{proposition:starreflection}. 
	This mapping and its inverse are piecewiselinear, and it acts as the identity along $\partial \Minako_{M-1} \setminus \partial T$.
