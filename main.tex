...
\newcommand{\mwl}[1]{{\color{red} MWL: #1}}
...
we call $n$-simplices $S,S' \in \calT$ \emph{face-connected in $\calT$} if there exists a sequence $S_0=S,S_1,\dots,S'=S_m \in \calT$ such that $S_{i} \cap S_{i-1}$ is a face of both $S_{i}$ and $S_{i-1}$ for all $1 \leq i \leq m$. % TODO: Theo suggests reverse order of that sequence 
...
%     \sup_{ \varphi : \Delta^n \rightarrow T } \matnorm{\Jacobian \varphi}_{\maxcol} \matnorm{ \Jacobian \varphi^{-1} }_{\maxcol}
    ,
\end{align}
...
%     \begin{align*}
%         \matnorm{ \Jacobian \varphi^{-1} }_{2} \leq \sqrt{n} \cdot \height(T)^{-1}
%         .
%     \end{align*}
...
\mwl{Throughout the manuscript, use a macro for opnorm and equip the matrices with the operator norm index.}
...
%         \\&
        ...
        \| \nabla u \|_{L^{p}(U)}
    \end{align*}
    for any $u \in W^{1,p}(\Omega)$ with $p \in [1,\infty]$. \mwl{Introduce constant $C_{{\rm{PF}},T_1 \cup T_2,p}$}
\end{lemma}
% \begin{remark}
...
\begin{proof}
    ...
    Write $\hat U := \Delta_1 \cup \Delta_2$, which is (crucially) convex. .
...
%     Hence 
    ...
    Thus, due to~\eqref{math:esposito:pf}, there exists $\hat w \in W^{1,p}(\hat U)$ such that $\nabla \hat w = \nabla \hat u$ and 
    \begin{align*}
        \| \hat w \|_{L^{p}(\hat U)}
        \leq 
        2 C_{\PF,{\rm EFNT},p}
    ...
    \begin{align*}
        ...
        .
    \end{align*}
    \cye{In the last equality, we have also used $|\det(\Jacobian\varphi_i)| = n! \vol(T_i)$ that we have already employed in~\eqref{equation:Hong_Pan}.}
    Noting $\vol(T_i) = |\det(\Jacobian \varphi_i)|$ and $\matnorm{ \Jacobian \varphi_i } \leq \Ceins{n} \diam(T_i)$ for $i = 1, 2$, where we use Lemma~\ref{lemma:measurerelationships}, we conclude the desired result.
\begin{comment}
...
\begin{theorem}\label{theorem:poincarefriedrichsestimate:grad}
    Let $\Omega \subseteq \bbR^{n}$ be a domain with a face-connected $n$-dimensional finite triangulation $\calT$.
    We fix an $n$-simplex $T_0 \in \calT$ and for all $n$-simplices $T \in \calT$ 
    we fix a strong path $T_0, T_1, \dots, T_{M} = T$.
...
%     DONE {Make sure the notation for the PF constants is uniform throughout the paper}
    \begin{align*} 
        ...
        .
    \end{align*}
    \todo{Rewrite this result. First a local recursive one, then a global bound}
    \todo{Ensure big $M$ at all indices here.}
...
%  Since $\calT$ is face-connected, there exists an enumeration of $n$-simplices $T_0, T_1, \dots, T_M$ such that for any $1 \leq m \leq M$ there exists some fixed but arbitrary $0 \leq j(m) < m$ such that $F_m := T_m \cap T_{j(m)}$ is a face of dimension $n-1$. We write $\Omega_m := \cup_{j=0}^{m} T_m$. Hence $\Omega = \Omega_M$. 
...
\begin{lemma}\label{lemma:mixedbconsimplex}
    ...
    \begin{align*}
        ...
        ,
    \end{align*}
    where $C_{\PF,T,F,p} = \frac{1}{\sqrt[p]{p}} \diam(T)$ for $p < \infty$ and $C_{\PF,T,F,\infty} = 1$. 
%     \begin{align*}
%         C_{\PF,T,F,p}
%         &\leq 
...
\end{comment}
    ...
    We trivially extend $u$ to a function over the entire $\bbR^{n}$.
...
%         \int_{\Omega} |\nabla u|^{p} \,dx
    ...
    There exists affine diffeomorphism $\varphi : \Delta^{n} \rightarrow T$ mapping the convex closure of the $n$ unit vectors onto the face $F$.
...
\begin{theorem}\label{theorem:poincarefriedrichsestimate:gradzwei}
    Let $\Omega \subseteq \bbR^{n}$ be a domain with a face-connected finite triangulation $\calT$.
...
%         \end{align*}
        ...
        Then $\vecnorm{ h_z } = \cos(\beta) \vecnorm{ z_{S'} }$ and $\vecnorm{ b_z } = \sin(\beta) \vecnorm{ z_{S'} }$. 
...
%         Let $\beta$ be the angle between $z_{S'}$ and $h_{z}$. 
        By the definition of the tangent, $\tan(\beta) \leq \vecnorm{ b_{z} } / \vecnorm{ h_{z} }$, and we know $\vecnorm{ b_z } \leq \vecnorm{ z_{S'} }$. 
...
%             \geq 
        ...
        \begin{align*}
            ...
            - 
            \frac{ \vecnorm{ b_z } }{ 2 \vecnorm{ h_{z} } } 
            \hat b_{z} \otimes \hat h_{z}^{t}
            .
        \end{align*}
        We write $\beta$ for the angle between $h_{z}$ and $z_{S'}$.
        Hence $\tan(\beta) = \vecnorm{ b_z } / \vecnorm{ h_z }$. It remains to study the singular values of the matrix 
        \begin{align*}
            C 
            = 
            \frac 1 2 
...
%         \end{align*}
        ...
        \begin{align*}
            \tan(\beta) = \frac{\vecnorm{ b_z }}{\vecnorm{ h_z }} \leq (k+1) \aspectratio(T).
        \end{align*}
        We conclude that 
        \begin{gather*}
