...
\title{Poincar\'e--Friedrichs constants for the $L^{p}$ de Rham complex\\ over domains with shellable triangulations}
\author{
...
\begin{abstract}
    ...
    We express all our main results in the calculus of differential forms and treat the curl and divergence operator as its particular instances. 
\end{abstract}
...
In particular, such operator norm bounds yield computable upper bounds for Poincar\'e--Friedrichs inequalities and computable lower bounds for the eigenvalues of several vector Laplacians. 
...
We accomplish this task in the special case where $\Omega$ admits a so-called \emph{shellable} triangulation. 
...
Notably, the optimal constants $C_{\grad,\Omega,p}$ for any $1 \leq p < \infty$ that depends only on the domain diameter is known when the domain is convex.~\cite{bebendorf2003note,acosta2004optimal,esposito2013poincare,ferone2012remark}.
When the domain is not convex but star-shaped with respect to a ball,
then polynomial interpolation estimates already imply the Poincar\'e inequality~\cite{brenner2008finite,ern2021finite}. 
...
This only requires that certain linear combinations of distributional partial derivatives are integrable, 
...
The main objective of this manuscript is bounding the Poincar\'e--Friedrichs constants over local patches within triangulated domains. 
...
Estimates that rely on this geometric condition thus deteriorate when the aforementioned ball has small radius,
...
% 
The literature~\cite{vohralik2005discrete,ern2021finite} knows different variations of a popular avenue towards Poincar\'e--Friedrichs constants that circumvents the effect of low boundary regularity and constructs potentials step-by-step: 
starting with a single triangle, the potential is constructed over increasing subdomains, until the entire domain is exhausted. That procedure applies to general triangulated domains, not only local stars, though the latter is our main interest. 
...
While we know Poincar\'e--Friedrichs constants over local patches for scalar-valued Sobolev spaces, 
...
Towards our main objective of constructing linear potential operators for the gradient, curl, and divergence, satisfying explicitly computable upper bounds, we follow closely the philosophy of the aforementioned sequential approach. Transitioning to vector fields incurs new challenges due to the infinite-dimensional kernels. We overcome these challenges on a special class of triangulated domains, namely domains with so-called \emph{shellable} triangulations. 

% Exterior calculus 
Though we present our results in vector calculus, our main arguments are given in the formalism of exterior calculus~\cite{greub1967multilinear,lee2012smooth}. Exterior calculus is used ubiquitously in the mathematical literature of physics and engineering and has found widespread adoption in the theoretical and numerical analysis for vector-valued partial differential equations~\cite{hiptmair2002finite,gross2004electromagnetic,arnold2006finite,arnold2009geometric,arnold2010finite,}. Loosely speaking, exterior calculus is the calculus of antisymmetric tensor fields. The formalism is independent of the dimension and highlights underlying geometric structures common to the gradient, curl, and divergence operators.
...
\section{Constructive estimate of Poincar\'e--Friedrichs constants}\label{section:poincare}

In this section we address the stepwise computable estimate of Poincar\'e--Friedrichs constants of triangulated domains. As a preparatory step, we survey variations of the Poincar\'e and Poincar\'e--Friedrichs inequalities over convex domains. This survey, which we believe to be of independent interest, will serve as a building block in constructing inequalities over triangulated domains.
\\
...
We wish to clarify the relationship between the Poincar\'e--Friedrichs inequality, in the sense introduced above, with other inequalities that are known as Poincar\'e inequality (or also Poincar\'e--Wirtinger or Friedrichs inequality) in the literature. Given $p \in [1,\infty]$ and an open set $\Omega \subseteq \bbR^{n}$ of finite measure, % TODO: cite AAAA
...
We conclude that taking the average is a projection within Lebegue spaces with unit norm. 
It follows that 
\begin{align*}
    \| u - u_\Omega \|_{L^{p}(\Omega)} 
    \leq
...
\end{align*}
In the special case $p=2$, taking the average is an orthogonal projection, and so this improves to $\| u - u_\Omega \|_{L^{2}(\Omega)} \leq \| u \|_{L^{2}(\Omega)}$ for any $u \in L^{2}(\Omega)$,
and therefore even $C_{{\rm P},\Omega,2} = C_{{\rm PF},\Omega,2}$. 
...
\end{align}
In summary, our notion of Poincar\'e--Friedrichs constant is equivalent to the notion of Poincar\'e constant in the literature, up to a numerical factor of at most two that only depends on $1 \leq p \leq \infty$.
\\
...
We will extend the potential reconstruction when we address Poincar\'e--Friedrichs inequalities for the curl and divergence operators. 
...
Upper bounds for the Poincar\'e constant over convex domains with $1 < p < \infty$ are known in the literature~\cite[Theorem 1.1]{chua2006estimates}:
...
\begin{align}\label{math:esposito:poincare}
    ...
    .
\end{align}
In the limit case $p=1$, the optimal Poincar\'e constant $1/2$ also bounds the Poincar\'e--Friedrichs constant:
\begin{align}\label{math:l1estimate}
    ...
    .
\end{align}
In the limit case $p=\infty$, since convex domains are Lipschitz domains, Rademacher's theorem leads to 
...
\begin{example}
    Let $\Omega = (-1,1)$ and consider the family of odd function $u_{t} : \Omega \rightarrow \bbR$ for $t \in (0,1)$ 
    that satisfy $u_{t}(0)=0$, $u_{t}(t) = 1$ and $u_t(1) = 1$ and interpolate linearly between those points. 
    Obviously,
    \begin{align}
        \| u_{t} \|_{L^{1}(\Omega)} = 2 - t,
        \qquad 
        \| u'_{t} \|_{L^{1}(\Omega)} = 2t \frac{2}{2t} = 2.
        ,
    \end{align}
    and $\Omega$ has diameter $2$. As $u_t$ has average zero, this example shows that \eqref{math:acostaduran} is optimal. 
    At the same time, for any constant $c \in \bbR$ we have $\| u_t - c \|_{L^1(\Omega)} = \| u_t + c \|_{L^1(\Omega)}$.
    Since norms are convex, it follows that $u_t$ is the potential with minimal $L^1$ norm. 
\end{example}
...
%         \frac 1 2
    ...
    which is the optimal Poincar\'e--Friedrichs constant and also the factor in Inequality \eqref{math:acostaduran}. 
...
\subsection{Poincar\'e--Friedrichs inequalities over triangulated domains}
...
\begin{align}\label{math:geometricshapemeasure}
    ...
    .
\end{align}
% TODO: Although this is fairly clear upon reading, we could specify in word that $\kappa(T)$ is (equivalent to) the usual shape regularity parameter, whereas $\rho(\mathcal T)$ is the maximal ratio of volumes between neighboring elements.
...
\begin{proof}
    ...
    Thus, $\sigma_1/\sigma_n \leq \sigma_1 / \sqrt[n]{ \det(\Jacobian\varphi) }$ and 
    \[
        \diam(T) 
        \leq 
        \sqrt{2} \sigma_1 
        \leq 
        \sqrt{2} 
        \frac{ \sigma_1 }{ \sqrt[n]{ \det(\Jacobian\varphi) } } 
        \sigma_n
        \leq 
        \sqrt{2n} 
        \left( \frac{ \diam(T)^{n} }{ n! \vol(T) } \right)^{\frac 1 n}
        \sigma_n
        .
    \]
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \diam(T_1)
        %
        % \sqrt{2n} \left( \frac{ n! \vol(T_2) } {\diam(T_2)^{n} } \right)^{\frac 1 n}
        \sqrt{2n} \kappa(T_2)^{\frac 1 n}
        \diam(T_2)^{-1}
        \\&
        \leq 
        \sqrt{n} 
        ...
        .
    \end{align*}
    All the desired results thus follow. % TODO: improve these results 
...
\begin{theorem}\label{theorem:poincarefriedrichsestimate:grad}
    ...
    \begin{align*}
        ...
        % 2 C_{\rm{EFNT},p} \rho(\calT)^{\frac 1 p} \max\left( \diam(T_{t}), \diam(T_{t-1}) \right)
        \| \nabla u \|_{L^{p}(T_{t} \cup T_{t-1})}
        \\&\quad \quad \quad 
        +
        \rho(\calT)^{\frac{m-1}{p}}
        % C_{\rm{EFNT},p} \diam(T_0)
        C_{{\rm{PF}},T_{0},p}
        \| \nabla u \|_{L^{p}(T_{0})}
        .
    \end{align*}
...
\begin{proof}
 ...
 we let $u_F \in W^{1,p}(U_F)$ such that $\nabla u_F = \nabla u$ over $U_F$ and such that $\| u_F \|_{L^{p}(U_F)} \leq C_{{\rm{PF}},U_F,p} \| \nabla u_F \|_{L^{p}(U_F)}$.
...
%  then 
 \begin{align*}
    ...
    .
 \end{align*}
 We use Definition~\eqref{math:moremeasures} to prove the desired inequality.
%  \begin{align*}
%     \| w_{m} \|_{L^{p}(T_m)}^{p}
%     &\leq 
%     (m+1)^{p-1}
...
\begin{proof}
    ...
    \[
        ...
        \leq 
        C_{{\rm PF},T,p} 
        \| \nabla w \|_{\bfL^{p}(T)} % TODO: bold face or no boldface? No bold face 
        .
    \]
...
\begin{proof}
    ...
    At the same time, $X = F \cap \partial \mathring B_F$ is empty if $n=1$ or homeomorphic to a sphere $S' \subseteq S$ of dimension $n-2$. % TODO: proofread
...
\begin{lemma}\label{lemma:startopology}
    ...
    \begin{itemize}
        ...
        and $\carapace_{\calT}(S)$ is a simplicial $(n-1)$-sphere. % TODO: Define this macro 
...
\begin{lemma}    
    If an $n$-dimensional simplicial complex $\calT$ has a shelling $T_{0}, T_{1}, T_{2}, \dots, T_{M}$ and is non-branching, % TODO: what is non-branching?
    then
    $$
...
\begin{proof}
    ...
    Write $M = |\calT|$. 
    We call a triangle $T \in \calT$ \emph{good} if it intersects the boundary $\partial M$ in a non-empty union of edges. % TODO: I understand what it means from the rest of the proof, but I believe it could a bit more explicit.
...
%
% TODO: bounds on the Jacobian transform 
%
...
\begin{align*}
    ...
    .
\end{align*}
If $x, y \in \Omega$, then we can restrict the inner integrals to the range $0 \leq r \leq D$, which gives % TODO: define D
\begin{align*}
    G_{\ell}(x,y) 
    &= 
