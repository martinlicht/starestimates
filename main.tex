...
% DONT: Keep ?
%     In the limit as $p$ goes to $1$, the factor $C_{{\rm PF},{\rm EFNT},p}$ converges to $1/2$,
...
%         .
%     \end{align*}
%     In the limit as $p$ goes to infinity, the factor $C_{{\rm PF},{\rm EFNT},p}$ converges to $1/2$, 
...
% DONE: Introduce a reference simplex 
% DONE: DEFINE THE JACOBIAN 
% \notice{MWL: If we need to define the Jacobian formally, where should that be put? Maybe an extra paragraph \textbf{Notation} immediately at the end of the introduction section.}
...
%     By a result of Hong and Pan,
%     \begin{align*}
%         \frac{ \sigma_{n} }{ \sigma_{1} }
%         \geq 
%         \left(\frac{n-1}{n}\right)^{\frac{n-1}{2}}
%         \frac{ \sqrt{n} \det(\Jacobian\varphi) }{ \| \Jacobian\varphi \| }
%         \sigma_{1}^{-1}
%         \geq 
%         \left(\frac{n-1}{n}\right)^{\frac{n-1}{2}}
%         \frac{ \sqrt{n} \det(\Jacobian\varphi) }{ n \sigma_{1} }
%         \sigma_{1}^{-1}
%         .
%     \end{align*}
...
%         (n-1)^{\frac{n-1}{2}}
%         \det(\Jacobian\varphi)
%         \left( \frac{1}{ \diam(T)^{n-1} } \right)^{}
%         \frac{1}{\sqrt{n} \diam(T)}
%         \\&
%         \geq 
%         \frac{1}{\sqrt{n}}
%         (n-1)^{\frac{n-1}{2}}
...
%     Thus, $\sigma_1/\sigma_n \leq \sigma_1^{n} / { \det(\Jacobian\varphi) }$ and 
%     \begin{gather*}
%         \diam(T) 
%         \leq 
%         \sqrt{2} \sigma_1 
...
%         \leq 
%         \sqrt{2n^{n}} 
%         \left( \frac{ \diam(T)^{n} }{ n! \vol(T) } \right)
%         \sigma_n
%         .
%     \end{gather*}
...
%     DONE {Make sure the notation for the PF constants is uniform throughout the paper}
    \begin{align*} 
        ...
        % 2 C_{{\rm PF},{\rm EFNT},p} \rho(\calT)^{\frac 1 p} \max\left( \diam(T_{t}), \diam(T_{t-1}) \right)
        \| \nabla u \|_{L^{p}(T_{t} \cup T_{t-1})}
        \\&\quad \quad \quad 
        +
        \rho(\calT)^{\frac{m-1}{p}}
        % C_{{\rm PF},{\rm EFNT},p} \diam(T_0)
        C_{{\rm{PF}},T_{0},p}
        \| \nabla u \|_{L^{p}(T_{0})}
        .
    \end{align*}
...
\begin{proof}
    ...
    At the same time, $X = F \cap \partial \mathring B_F$ is empty if $n=1$ or homeomorphic to a sphere of dimension $n-2$. % TODO: proofread
...
%     \todo{More details}
%     {\color{blue}
%     The doubling $M_{D}$ of the manifold $M$ is obtained by gluing $M$ to a copy of itself along the boundary.
%     The triangulation $\calT$ can be doubled to get a triangulation of the doubled manifold.
%     Any boundary face is an interior face of the doubled manifold two $n$-simplices of the doubled triangulation.
%     So each face within $\partial \calM$ is contained in exactly one $n$-simplex of $\calT$. 
%     }
...
\begin{proof}
    ...
    %\todo{Double check this proof and make it more explicit.}
    Write $M = |\calT|$. 
    ...
    % TODO: I understand what it means from the rest of the proof, but I believe it could a bit more explicit.
    ...
    % \todo{MV:But what if this is the neighbor of $T'$ and adding $T'$ back makes the triangle bad?}
...
\begin{lemma}\label{lemma:existenceofstar}
    ...
    then the intersection of those facets is a simplex $S \in \calT$ of dimension $n-k$ that satisfies 
    $$
...
\begin{proof}
    ...
    By assumption, $S$ must be a boundary simplex of $X_{m+1}$. 
    \todo{Complete this here. The barycenter is contained in the interior of a boundary disk.}
\end{proof}
...
Every time add a simplex according to the shelling sequence, 
we complete the local star of the some simplex in the triangulation.
We search for a homeomorphism from the new simplex on its complement in the newly completed star
such that the interface is preserved. 
We want this homeomorphism to be bi-Lipschitz and we are particularly interested in the norms of its Jacobian.
...
\begin{theorem}\label{theorem:starreflection}
    ...
    Let $T \in \patch_{\calT}(S)$ be of dimension $n$ and let 
    \begin{align*}
        M := |\patch_{\calT}(S)| \setminus T.
    \end{align*}
    ...
    satisfying
    \begin{align*}
        \Lip( \varphi,      T ) \leq \Cvier{n}{k} \kappa(\calT)^{2},
        \quad 
        \Lip( \varphi^{-1}, M ) \leq \Cfive{n}{k} \kappa(\calT)^{2},
    \end{align*}
    where $\Cvier{n}{k} \leq ...$ and $\Cfive{n}{k} \leq ...$

    \todo{Complete the estimate.}
\end{theorem}
\begin{proof}
    ...
    \begin{itemize}
        ...
        \begin{align}
            ...
            .
        \end{align}
        The Euclidean operator norm is 
        \begin{align}
            \| \Jacobian F_T(x) \| 
            \leq 
            2 \left| \vecone \cdot A_{T}^{-1} x \right|
            \leq 
            2 \sqrt{n} \cdot \sigma_{\max}(A_{T}^{-1}) 
            .
        \end{align}
        \color{black}
        ...
        \begin{align*}
            ...
            \right\|_{2}
            +
            \left| \|A_{T}^{-1} x\|_2 - \|A_{T}^{-1} y\|_2 \right|
            \\&\qquad 
            \leq 
        ...
        \begin{align*}
            ...
            +
            \left\| 
                \frac{ \|x\|_2 }{ \| A_{T}^{-1} x \|_{1} } y - \frac{ \|y\|_2 }{ \| A_{T}^{-1} y \|_{1} } y
            \right\|_{2}
            \\&\qquad 
            \leq 
            ...
            \frac{ \|y\|_2 }{ \| A_{T}^{-1} y \|_{1} }
            \\&\qquad 
            \leq 
            2 \sigma_{\min}(A_{T}^{-1})^{-1} 
            \left\| x - y \right\|_{2}
            .
        \end{align*}
        ...
        Let $I := M \cap T$, which is the union of $k$ different faces of $T$, with $1 \leq k \leq n$.
        We let $J \subseteq \partial T$ be the union of the remaining $n+1-k$ faces of $T$.
        So $J$ is in the boundary of $|\calT|$ and is disjoint from $S$.
        ...
        \item 
        We construct a new triangulation that contains $T$ and that is a star around $S$.
        For that purpose, we let $z'_{S}$ be the midpoint of $S'$. 
        We first define a new triangulation $\calS$ by the barycentric refinement of $T$. 
        ...
        \item 
        % \todo{Reflect onto complement and estimate bi-Lipschitz constant}
        ...
        It follows that 
        \begin{align*}
            \Theta_{S} := \Psi_{\calK}^{-1} \Psi_{\calR'} \Theta
        \end{align*}
        ...
        \begin{align*}
            ...
            \leq 
            \sup_{ K \in \calK  }
            2 \sigma_{\min}(A_{K}^{-1})^{-1} 
            \sup_{ K \in \calR' } 
            \left( \sqrt{n} + 1 \right) \sigma_{\max}(A_{K}^{-1}) 
        ...
        \begin{align*}
            ...
            \| \Jacobian \Theta_{S} \| 
            &\leq 
            2 \sup_{ K \in \patch_{\calT}(S) }
            2 \sigma_{\min}(A_{K}^{-1})^{-1} 
            4 \sup_{ K \in \patch_{\calT}(S) } 
            \left( \sqrt{n} + 1 \right) \sigma_{\max}(A_{K}^{-1}) 
            4 \sup_{ K \in \patch_{\calT}(S) } 
            \kappa(A_{K})
            \\&\leq 
...
\begin{remark}
    ...
    it might be possible to improve the estimate with a different analysis or construction.
\end{remark}
...
\begin{lemma}\label{lemma:PFexteriorsimplex}
    ...
    Let $0 \leq k < n$. 
    Then for each $\bfu \in \bfW^{p}\Lambda^{k}(\Omega)$ 
    there exists $\bfw \in \bfW^{p}\Lambda^{k}(\Omega)$ 
    with $\cartan \bfu = \cartan \bfw$ and 
    \begin{align*}
        \| \bfw \|_{L^{p}(\Omega)}
        &
        \leq 
...
\begin{lemma}\label{lemma:mixedbconsimplex:exteriorderivative}
    ...
    Suppose that $\bfu \in \bfW^{p}\Lambda^{k}(T)$ such that 
    $\trace_{T,F_i} \bfu = 0$ for $0 \leq i \leq l$.
    Then there exists $\bfw \in \bfW^{p}\Lambda^{k}(T)$ such that 
    $\trace_{T,F_i} \bfw = 0$ for $0 \leq i \leq l$
    and  
    \begin{align*}
        \cartan \bfu = \cartan \bfw,
        \quad 
        \| \bfw \| 
        \leq 
        C_{{\rm PF},T,F,k,p}
...
\begin{proof}
    ...
    We let $\hat u := \varphi^{\ast} u$ and define $\hat g \in \bfL^{p}(\hat T)$ via $\hat g := \cartan \varphi^{\ast} u$. 
    ...
    By construction, $\tilde u \in \bfW^{p}_{0}\Lambda^{k}(\tilde U)$.
    
    The bounds for the potential operators imply 
    the existence of $\tilde w \in \bfW^{p}_{0}\Lambda^{k}(\tilde U)$
    with $\cartan \tilde w = \cartan \tilde u$ and 
    \begin{align*}
        \| \tilde w \|_{L^{p}(\tilde U)}
        &\leq 
        n A_{\Bogov}(n,k+1) \frac{ \vol_{n}(B_2(0)) }{|\Omega|} 
        2
        \| \tilde g \|_{L^{p}(\tilde U)}
        \\&\leq 
        2^{n+1} n A_{\Bogov}(n,k+1) \frac{ \vol_{n}(B_1(0)) }{|\Omega|} 
        \| \tilde g \|_{L^{p}(\tilde U)}
        .
    \end{align*}
    We let $\bfw \in \bfW^{p}\Lambda^{k}(T)$ be defined by $\bfw := \varphi^{-\ast} \tilde w$.
    By construction, $\cartan \bfw = \cartan \bfu$,
    and $\bfw$ has vanishing trace along the faces $F_{0},\dots,F_{l}$.
...
\begin{proof}
    ...
    In accordance to Lemma~\ref{lemma:existenceofstar}, adding $T_{m}$ completes a star in $\calT$ around some simplex, and we let $U_{m} \subseteq \Omega_{m}$ be the complement of $T_{m}$ in that newly completed star. 
    ...
    By the properties of the reflection mapping in Theorem~\ref{theorem:starreflection},
    \begin{align*}
        ...
    \end{align*}
