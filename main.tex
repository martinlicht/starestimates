...
We achieve this goal via several auxiliary results. 
We start with a few geometric observations that pertain to the mesh regularity. 
Whenever $T$ is an $n$-dimensional simplex, we define the quantity 
\begin{align*}
    \mu(T)
    := 
...
\begin{align*}
    \mu(\calT) := \sup_{ T \in \subsimplex_{n}(\calT) } \mu(T),
    \qquad 
    \delta(\calT) := \sup_{ \substack{ T, T' \in \subsimplex_{n}(\calT) \\ T \cap T' \in \subsimplex_{n-1}(\calT) } } \frac{ \diam(T) }{ \diam(T') },
    \qquad 
    \kappa(\calT) := \sup_{ \substack{ T, T' \in \subsimplex_{n}(\calT) \\ T \cap T' \in \subsimplex_{n-1}(\calT) } } \frac{ \vol(T) }{ \vol(T') }.
\end{align*}


\begin{lemma}\label{lemma:affinetransform}
    Let $T$ be an $n$-simplex and let $\Psi : \Delta^n \rightarrow T$ be an affine diffeomorphism. Then 
    \begin{align*}
        \| \Jacobian \Psi \|
        \leq 
        \sqrt{n} \cdot \diam(T)
...
\end{lemma}
\begin{proof}
    We easily verify the first inequality. To prove the second inequality, let $\sigma_1 \geq \dots \geq \sigma_n \geq 0$ denote the singular values of $\Jacobian\Psi$ in descending order.
    Then $\sigma_1/\sigma_n \leq \sigma_1^n / \det(\Jacobian\Psi)$. Hence 
    \[
        \diam(T) 
        \leq 
        \sqrt{2} \sigma_1 
        \leq 
        \sqrt{2} 
        \frac{ \sigma_1^n }{ \det(\Jacobian\Psi) } 
        \sigma_n
        \leq 
        \sqrt{2n^n} 
        \frac{ \diam(T)^n }{ n! \vol(T) } 
...
\begin{remark}
    ...
    Whenever $T$ is an $n$-dimensional simplex, the ratio $\mu(T)$ measures the ``shape quality'' of a simplex and is instance of a so-called \emph{shape measure}.
...
\begin{proof}
    ...
    We let $\hat u := u \circ \Psi$ and define $\hat g \in \bfL^p(\hat T)$ via $\hat g := \nabla ( u \circ \Psi )$. 
    Then $\nabla \hat u = \hat g$. 
    % 
    Let $\hat U$ be the unit ball of the $\ell^1$ metric, which contains $\hat T$.
    We let $\tilde u$ be the extension of $\hat u$ onto $\hat U$ by reflection across the coordiante axes.
    We define $\tilde g = \nabla \tilde u$
    ...
    \begin{gather*}
        ...
        \leq 
        2^{\frac n p}
        |\det(\Dif \Psi_1)|^{-\frac 1 p} \| \Dif \Psi \| \cdot 
        \| \nabla u \|_{\bfL^{p}(T)}
        .
    \end{gather*}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        + 
        C_{{\rm PF},T,F,p} 
        \left( 1 + \kappa(\calT)^{\frac 1 p} \xi(\calT) \right) 
        \diam(T_{m+1}) 
        \| \nabla u \|_{L^{p}(T_{m+1})} 
        .
    \end{align*}
