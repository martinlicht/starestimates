...
% and $\Phi : \Delta^{n} \rightarrow T$ is an affine diffeomorphism from the reference simplex,
...
\begin{proof}
 ...
 We write $\Omega_m := \cup_{j=0}^{m} T_m$, which is a triangulated \textbf{domain}. % TODO: technical stuff 
...
\subsection{Bogovski\u{\i}-type operators}
We begin with introducing the Costabel-McIntosh-Bogovski\u{\i} kernel.
For $\ell\in\{0,\dotsc,n\}$, we define the kernel $G_\ell : \bbR^{n} \times \bbR^{n} \rightarrow \bbR$ by
\begin{equation}
\label{eq:Gl}
  G_\ell(x,y) = 
  \int_1^\infty (t-1)^{n-\ell}t^{\ell-1} |\Omega|^{-1}\chi_{\Omega} \left(y+t(x-y)\right)\,dt \;.
\end{equation}
Given a differential form $u \in {C^\infty}_0(\bbR^n,\Alt^\ell)$, \(1\leq \ell \leq n\), 
we then define the integral operators
\begin{align}
  T_\ell u(x) &= \int G_{\ell}(x,y) \,(x-y)\lrcorner u(y)\,dy
\end{align}
We call $T_\ell$ a Bogovski\u{\i}-type operator. 

In order to see that the integrals in the definition of $T_\ell$ exist, 
we first rewrite the kernel $G_\ell$:
\begin{align*}
    G_\ell(x,y) 
    &= 
...
\end{align*}
We pointwise estimate $B_{\ell} u(x)$ by the result of a convolution of a locally integrable function with $u$:
\begin{align*}
    \left| B_{\ell} u(x) \right|
    &=
    \left| 
...
Taylor's inequality now implies: 
\begin{align*}
    \| B_{\ell} u \|_{L^{p}(\Omega)}
    &\leq 
    ...
    .
\end{align*}
In particular, we have a bounded operator $B_{\ell} : L^{p}(\bbR^n,\Alt^{\ell}) \rightarrow L^{p}(\bbR^n,\Alt^{\ell-1})$ for any $1 \leq p \leq \infty$.  
...
\begin{align*}
    T_\ell u(x) 
    &= 
    \int \int_1^\infty (t-1)^{n-\ell}t^{\ell-1} 
    ...
    \\
    &= 
    \int \int_1^\infty (t-1)^{n-\ell}t^{\ell-1} (t+1)^{n}
...
\begin{align*}
    T_\ell u(x) 
    &= 
    \int \int_1^\infty (t-1)^{n-\ell} \left( 1/s-1 \right)^{\ell-1} (t+1)^{n}
    \chi_{\Omega}\left(a\right) 
    \frac{ x-a }{t+1}\lrcorner u( x + (a-x)/(t+1) ) \,dt\,dy 
    \\
    &= 
...
\begin{align*}
    &= 
    \int \int_1^\infty (t-1)^{n-\ell}t^{-n+\ell-1} 
    \chi_{\Omega}\left(a\right)
    \frac{(a-y)}{t} \lrcorner u(x+(a-x)/t) \,dt\,da 
    \\
    &= 
    \int \int_1^\infty (t-1)^{n-\ell}t^{\ell-1-n-1} \chi_{\Omega}\left(a\right)\, (x-a)\lrcorner u(x+(a-x)/t) \,dt\,da
    .
\end{align*}
Then we substitute $s = (t-1)/t$:
\begin{align*}
    T_\ell u(x) 
    &= 
    \int \int_1^\infty s^{n-\ell} (s-1)^{2} \chi_{\Omega}\left(a\right)\, (x-a)\lrcorner u( x+(s-1)(a-x) ) \frac{-1}{(s-1)^2}\,ds\,da
    \\
    &= 
    - \int \int_1^\infty s^{n-\ell} \chi_{\Omega}\left(a\right)\, (x-a)\lrcorner u( s(a-x) - a ) \,ds\,da
    \\
    &=
    - \int \chi_{\Omega}(a) \,(x-a)\lrcorner \int_1^\infty t^{\ell-1} u\left(a+t(x-a)\right)\,dt\,da\;
    .
\end{align*}
We copy a statement by McIntosh and Costabel, who are too fucking lazy to write down their goddamn substititions for fucks sake, stupid assholes:
\begin{align*}
    T_\ell u(x) 
    = 
    - \int \chi_{\Omega}(a) \,(x-a)\lrcorner \int_1^\infty t^{\ell-1} u\left(a+t(x-a)\right)\,dt\,da\;
...
$a = y + t(x-y)$
$s = \frac{t}{t-1}$
From that representation we can tell that whenever $u \in C^{\infty}(\bbR^{n})$ has support contained in $\overline\Omega$, we immediately have that $T_\ell u(x)$ is smooth with support contained in $\overline\Omega$ as well. 

We compute the exterior derivative of this potential:
\begin{align*}
    T_{\ell,a} u(x) 
    &= 
    (x-a)\lrcorner \int_1^\infty t^{\ell-1} u\left(a+t(x-a)\right)\,dt\,da\;
    \\&
    = 
    (x-a)\lrcorner 
    \sum_{\sigma \in \Sigma(k,n)}
...
\begin{align*}
    ...
    \int_1^\infty 
    t^{\ell} \partial_{j} u_{\sigma}\left(a+t(x-a)\right) (-1)^{i-1} (x-a)_{\sigma(i) } \cartanx^{j} \cartanx^{\sigma-\sigma(i)} dt 
    .
\end{align*}
When we calculate the exterior derivative of $u$, we find 
\begin{align*}
    \cartan u(x)
    =
    \sum_{\sigma \in \Sigma(k,n)} \sum_{j=1}^{n}
    \partial_{j} u(x) \cartanx^{j} \wedge \cartanx^{\sigma} dt 
    .
\end{align*}
The potential operator, applied to this result, gives 
\begin{align*}
    T_{\ell+1,a} \cartan u(x)
    &=
    (x-a)\lrcorner 
    \sum_{\sigma \in \Sigma(k,n)} \sum_{j=1}^{n}
    \int_1^\infty t^{\ell} \partial_{j} u_{\sigma}\left(a+t(x-a)\right) dt 
    \cartanx^{j} \wedge \cartanx^{\sigma}
    \\&
    = 
    \sum_{\sigma \in \Sigma(k,n)} \sum_{j=1}^{n}
    \int_1^\infty t^{\ell} \partial_{j} u_{\sigma}\left(a+t(x-a)\right) dt (x-a)_{j}
    \cartanx^{\sigma}
    - 
    \sum_{\sigma \in \Sigma(k,n)} \sum_{i=1}^{k} \sum_{j=1}^{n}
    (-1)^{i-1}
...
\begin{align*}
    ...
    =
    \sum_{\sigma \in \Sigma(k,n)} 
    \int_1^\infty \partial_{t} \left( t^{\ell} u_{\sigma}\left(a+t(x-a)\right) \right) dt \cartanx^{\sigma}
    \\&
    =
    \sum_{\sigma \in \Sigma(k,n)} 
    u_{\sigma}\left(x)\right) \cartanx^{\sigma}
    =
    u(x)
    .
\end{align*}
...
From this form of $T_\ell$, because of the unbounded interval of integration in $t$, one cannot immediately conclude that $T_\ell$ maps ${C^\infty}$ functions to ${C^\infty}$ functions.
But if $u\in{C^\infty}_0(\bbR^n,\Alt^\ell)$, one sees that $T_\ell u$ is ${C^\infty}$ on 
$\bbR^n\setminus\supp\chi_{\Omega}$, and that 
$T_\ell u(x)=0$ unless $x$ lies in the starlike hull of $\supp u$ with respect to $B$. 
Thus if $\Omega$ is open and starlike with respect to $B$, then   
$u\in{C^\infty}_0(\Omega,\Alt^\ell)$ implies $\supp T_\ell u \subset\Omega$, and, if $\Omega$ is bounded, then
$u\in{C^\infty}_{\overline\Omega}(\bbR^n,\Alt^\ell)$ implies $\supp T_\ell u \subset\overline\Omega$. 
The fact that $T_\ell$ indeed maps 
${C^\infty}_0(\bbR^n,\Alt^\ell)$ to ${C^\infty}_0(\bbR^n,\Alt^{\ell-1})$ will be a consequence of Theorem~\ref{T:pseudo} below.
...
we find the homotopy relations, valid for all $u\in{C^\infty}_0(\bbR^n,\Alt^\ell)$ 
\begin{equation}
\begin{aligned}
\label{eq:dR+Rd=1}
...
\end{aligned}
\end{equation}
One could be tempted to integrate Cartan's formula from $1$ to $\infty$ and compare with \eqref{eq:Bogo}, thus formally obtaining a similar homotopy relation for $T_\ell$ directly. The result is indeed true except for $\ell=n$, but for a rigorous proof we prefer to use the duality relation \eqref{eq:dualRT} to deduce corresponding anticommutation relations for $T_\ell$ from the relations \eqref{eq:dR+Rd=1} which are already proved. Here is what one obtains for $u\in{C^\infty}_0(\bbR^n,\Alt^\ell)$:
\begin{equation}
\begin{aligned}
\label{eq:dT+Td=1}
  dT_\ell u + T_{\ell+1} du &= u\; &&(1\le\ell\le n-1)\;;\\
  T_1 du &= u \; &&(\ell=0)\;;\\
 dT_n u &=u - (\int\! u)\star\chi_{\Omega}&&(\ell=n)\;.
\end{aligned}
\end{equation}
Here we consider $\chi_{\Omega}$ as an element of ${C^\infty}_0(\bbR^n,\Alt^0)$, so that for another $0$-form $u$ we have the $L^2$ scalar product $\left(\chi_{\Omega},u\right)=\int\chi_{\Omega}(a)u(a)da$, and $\star\chi_{\Omega}$ is the $n$-form $\chi_{\Omega}(x)dx_1\wedge\dots\wedge dx_n$. 
...
If we correspondingly extend the definitions of $R_\ell$ and $T_\ell$ by
$$
\begin{aligned}
...
\begin{equation}\label{eq:R&T&d}
 ...
 \quad \text{ and }\quad
 \underline d\,T_\ell u + T_{\ell+1} \,\underline du = u\quad \text{ for all }\;
  0\le\ell\le n.
\end{equation}
