...
\begin{theorem}\label{theorem:globalestimate:gradient}
    Let $\calT$ be a face-connected $n$-dimensional finite triangulation
    and that the domain $\Omega$ is the interior of the underlying set of $\calT$. 
    ...
    \begin{gather*}
        0 = \imath_{m,0}, \; \imath_{m,1}, \; \dots, \; \imath_{m,L_m} = m
    \end{gather*}
    ...
    \begin{align*}
        ...
        + 
        B''_{m,\ell}
        \| \nabla u \|_{L^{p}(T_{m,\ell-1})}, 
        \qquad 1 \leq \ell \leq L_m
        .
    \end{align*}
    % TODO: complete here. We need a spanning tree. 
\end{theorem}
\begin{proof}
...
\begin{remark}\label{remark:poincarefriedrichsestimate:exterior:contraction}
    We would like to point out an alternative application of Proposition~\ref{proposition:starreflection},
...
We assess the practical quality of our upper bounds with a few numerical examples in dimension 2D and 3D, with exclusive focus on the Hilbert space case $p=2$. 
We want to compare the exact Poincar\'e--Friedrichs constants with the upper bounds given by Theorem~\ref{theorem:poincarefriedrichsestimate:exterior}.
In lieu of the exact value, finite element computations on a refined mesh provide reference eigenvalues as a proxy for the exact value.
We have multiple estimates for the gradient potential, and the Poincar\'e--Friedrichs constant of the divergence can always be estimated by the diameter,
hence our main interest is the curl operator. 
...
This example conforms to the well-known fact that the eigenvalues of the scalar Laplacian with mixed boundary conditions are between the Neumann and Dirichlet eigenvalues, which follows from a known variational principle. However, such an enclosure does not seem to hold for the vector Laplacians with mixed boundary conditions.

Lemma~\ref{lemma:mixedbconsimplex:exteriorderivative} and the subequent remark (using $p=2$) give an upper bound of $2/\pi \approx 0.45015815807$,
which is an overestimate by a factor of at most six.
...
We compare the reference Neumann and Dirichlet eigenvalues with the bounds computed in Theorem~\ref{theorem:poincarefriedrichsestimate:exterior}. Here, we take into account Lemma~\ref{lemma:volumecomparison} and Theorem~\ref{proposition:starreflection} (with Remark~\ref{remark:improved_starreflection}).
