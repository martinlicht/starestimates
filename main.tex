...
% TODO: write about the software and the algorithms 

% spanning trees 
Computing the upper bound for the Poincar\'e constant of Theorem~\ref{theorem:fullrecursivesum:grad} requires to find paths between simplices. One easily sees that the estimate is minimized by a spanning tree of the face-connection graph. 
We choose a depth-first search that attempts to minimize the resulting constant. 
While this does not take much time on small two-dimensional examples, the computation takes considerably in our three dimensional examples. 
...
The candidate shellings that we obtain in this manner lead to different quantitative estimates. In the case of more complicated 3D triangulations, such as of the Fichera corner, they may differ by a factor of $20$.
For that reason, optimizing the estimate induced by the shelling is an important computational subproblem. 

For tight estimates, it is imperative that we compute the relevant geometric parameters, such as heights, individually for each simplex and use individualized estimates for each of the geometric transformations. The estimates that we use are slightly tighter than the ones stated in Theorems~\ref{theorem:poincarefriedrichsestimate:grad} and~\ref{theorem:poincarefriedrichsestimate:exterior},
namely the calculations used in the proofs.
...
then we can replace the factor by $1/j_{1,1} \approx 0.2609803592$, where $j_{1,1}$ is the zero of a Bessel function; see~\cite{todo}.
...
A brufe-force search easily enumerates all the (repetition-free) face-paths between triangles, which allows us to find the best estimate possible estimate with the aforementioned theorem. 
%
...
Again, brute-force search is feasible to enumerate all possible shellings of moderately small two-dimensional triangulations, allowing us to compute the best possible of these bounds. 
...
\begin{table}[t]
    ...
    \begin{tabular}{|c||c|c|c|c|c||c|c|c|}
        ...
        \hline
        $\calT_{Q}$     & 0.318        & 1.684337597 &               & 1.684337582       & 3.141        & 0.225              & 1.684337545        &  6.928              \\ 
        \hline
        $\calT_{L}$     & 0.822        & 2.904309304 &               & 1.733455376       & 2.895              & 0.322              & 1.733455385              &  7.402              \\ 
        \hline
        $\calT_{S}$     & 0.978        & 3.357380776 &               & 2.170240475       & 2.982              & 0.346              & 2.170240474              &  8.419              \\ 
        \hline
        $\calT_{S'}$    & 0.978        & 3.368675195 &               & 3.368675191       & 4.870              & 0.346              & 3.368675191              & 13.748              \\ 
        \hline
    \end{tabular}
    \caption{
...
Enumerating all the shortest-path trees of the triangulations above is still feasible but takes considerably longer. 
However, brute-force enumeration of all shellings seems infeasible, though, due to the combinatorial explosion of the number of possible shellings. 
Instead, we perform a greedy backtracking search: 
starting with a single simplex, we successively increase the number of shellings until a complete shelling is found. 
Among the possible candidate simplices, we prioritize those that introduce the smallest possible geometric constant. 
We moreover prioritize the completion of face stars, since the induced constants are easier to control via Lemma~\ref{lemma:volumecomparison}. 
...
Our implementation starts with a fixed simplex and generates up to $10$ shellings. 
We repeat this for a possible choices of initial simplex, and pick the optimal shelling among those found.
The candidate shellings that we obtain in this manner lead to different quantitative estimates. In the case of more complicated triangulations, such as of the Fichera corner, they may differ by a factor of $20$.
For that reason, optimizing the estimate induced by the shelling is an important computational subproblem. 
