...
We gather a few helpful properties of simplicial manifolds. 
...
\begin{proof}
    \begin{enumerate}
    \item 
    We will use the following fact: if $S \in \calT$ has an inner point that lies on $\partial M$, then all inner points of $S$ are on $\partial M$. 
    Since the boundary is closed, every $S \in \calT$ is either a subset of the boundary or all its inner points lie in the interior of the manifold. 
    ...
    If $z_F$ is an interior point of $M$, 
    ...
    If $F$ is contained in only one $n$-simplex, then it must be a boundary face. 
    If a boundary face $F$ were contained in at least two $n$-simplices,
    then we could construct a homeomorphism from the $n$-ball onto itself that maps an interior point on the its boundary, which cannot be.
    So each boundary face is contained in exactly one $n$-simplex. 
    
    \item
    Clearly, the simplices of $\calT$ contained in the boundary constitute a simplicial complex. 
    If $x \in \partial M$ is a boundary simplex, then it is an inner point of some simplex $S \in \calT$.
    But then this simplex must be a boundary simplex, so the boundary simplices form a triangulation of $\partial M$.
    
    
    \item 
    % TODO structure of stars around simplices 
    Let $S$ be any simplex with midpoint $z_F$. 
    For $\delta > 0$ small enough, 
    \begin{align*}
        B_{\delta}(z_F) = \left\{ x \in M \suchthat d(x,z_F) \leq \delta \right\},
        \quad 
        S_{\delta}(z_F) = \left\{ x \in M \suchthat d(x,z_F) = \delta \right\},
    \end{align*}
    are subsets of $\patch_{\calT}(S)$.
%     For any $K \in \patch_{\calT}(S)$ that contains $S$, 
%     we can easily find a homeomorphism 
    
%     Then $\patch_{\calT}(S)$ is star-shaped with respect to $z_F$. 
%     We let $B$
%     Any half-ray starting at $z_F$ intersects at most one vertex in $\patch_{\calT}(S)$ other than $z_F$. 
%     
