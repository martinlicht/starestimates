...
\begin{proof}
    ...
    \color{red} yet to be completed.
\end{proof}


The following result by Veeser and Verfürth~\cite[Lemma 2.8]{veeser} is important in this context. 
...
%  There exists We already have 
 ...
 \begin{align*}
    ...
    .
 \end{align*}
 In the special case $p=\infty$, we get 
 \begin{align*}
    \| w_{m} \|_{L^{\infty}(T_m)}
    \leq 
    2
    ...
    .
 \end{align*}
 In particular, letting $q$ be the complementary exponent, 
 \begin{align*}
    \| w_{m} \|_{L^{p}(T_m)}
    \leq 
 ...
 \begin{align*}
    ...
    \left( 
        \prod_{t < l \leq m} 
        3^{\frac{p-1}{p}}
        \frac{ \vol(T_l)^{\frac 1 p} }{ \vol(T_{j(l)})^{\frac 1 p} }
    \right)
    \max\left( 1, \frac{ \vol(T_t) }{ \vol(T_{j(t)}) } \right)^{\frac 1 p}
    \| u_{F_t} \|_{L^{p}(U_{F_t})}
    \\&\quad \quad \quad 
    +
    \left( 
