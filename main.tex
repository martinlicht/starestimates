...
% TODO: MV : this first paragraph is a better abstract 
...
% TODO : MV : remove 'For example' 
For example, if $\bff \in \bfL^{2}(\Omega)$ is a square-integrable vector field on some domain $\Omega \subseteq \bbR^3$, 
we want to find a potential $\bfu \in \bfL^2(\Omega)$ under the curl operator, i.e., $\curl \bfu = \bff$, such that the $\bfL^2$ norm of the potential is bounded in terms of the source field $\bff$. This is expressed via the Poincar\'e--Friedrichs inequality 
\begin{align*}
...
% TODO: MV : Be more didactical: write following as the existence of a potential. 
...
\begin{align}\label{math:intro:poincarefriedrichs:grad}
    ...
    C_{\grad,\Omega,p} \| \nabla u \|_{L^p(\Omega)},
    \quad 
    u \in W^{1,p}(\Omega) % TODO: Use \forall notation 
    .\tag{PF} % TODO Decide later to keep the tags
\end{align}
...
\begin{align}\label{math:intro:poincare:grad}
    ...
    \quad 
    u \in W^{1,p}(\Omega)
    , \tag{P} % TODO: tag poincare inequality 
\end{align}
...
Notably, the optimal constants $C_{\grad,\Omega,p}$ for any $1 \leq p < \infty$ that depend only on the domain diameter are known when the domain is convex.~\cite{bebendorf2003note,acosta2004optimal,esposito2013poincare,ferone2012remark}.
...
Some illustrative limit cases include the slit domain~\cite{veeser2012poincare} and the crossed bricks domain~\cite{licht2019smoothed}, % DONE: MV : find the examples in the paper by verfuhrt and veeser 
...
% Exterior calculus % TODO: MV recommends moving this down to finish the current line of thoughts 
...
We review basic notions of triangulations in Section~\ref{section:triangulations} and provide computable upper bounds for Poincar\'e constants for the gradient over convex and triangulated domains in Section~\ref{section:poincare}.
...
We say that a simplicial complex $\calT$ has \textit{dimension $n$} or is \textit{$n$-dimensional} if each of its simplices is a subset of an $n$-dimensional member of that complex.\footnote{Simplicial complexes that we call $n$-dimensional are called purely $n$-dimensional in the literature on polytopes~(cf.\ \cite{ziegler2012lectures}).} 
...
\begin{remark}
    ...
    % 
    However, none of these infinite families are infinite simplicial complexes in the sense of topology~\cite{spanier1995algebraic}. 
\end{remark}
...
\section{Constructive estimates of Poincar\'e--Friedrichs constants}\label{section:poincare}
...
% TODO: MV move this part to the introduction, as an alternative the first few paragraphs 
...
%         \frac 1 2
        .
    \end{align*}
    It follows that \eqref{math:esposito:poincare} is a possibly tighter estimate than \eqref{math:chuawheeden} for $1 < p < \infty$.
    % TODO: 
    ...
    \begin{align*}
        ...
        %
    \end{align*}
    because the $L^{\infty}$-norm is the limit of the $L^{p}$-norm. % TODO: cite 
    % TODO: review this entire result 
\end{remark}
...
% TODO: as for instance recently used in \cite{Ern_Voh_p_rob_3D_20, Chaum_Voh_p_rob_3D_H_curl_23, Voh_loc_glob_H1_24}.
...
% 
% 
% TODO: Note that \kappa(T)=1 whenever T is a line segment, an isosceles right-angled triangle, or their generalizations to higher dimensions (multiples of a reference simplex). \kappa(T) is slightly bigger than one for equilateral simplices and blows up with distortion.
% 
% 
...
\begin{lemma}\label{lemma:affinetransform}
    ...
    \begin{align*}
        \| \Jacobian \varphi \|
        \leq 
        \sqrt{n} \cdot \diam(T) % TODO: can this improved?
        ,
        \qquad 
        \| \Jacobian \varphi^{-1} \|
        \leq 
...
\begin{proof}
    ...
    \[
        ...
        .
    \]
    The desired result is thus evident. % TODO: this is incorrect. Find a better explanation. 
\end{proof}
