...
We call such a sequence a \emph{strong path} from $S$ to $S'$ in $\calT$. \mwl{Change the terminology strong path to something else.}
...
\mwl{We should use clarify face vs facet}
...
\begin{remark}
    Lemma~\ref{lemma:volumecomparison} fully controls the shape quantity $\volumeratio(\calT)$ \cye{of~\eqref{math:volumeratio}} but not $\diameterratio(\calT)$ \cye{of~\eqref{math:diametercomparison}}. 
...
\mwl{In this section, we develop a stepwise computable estimate of Poincar\'e--Friedrichs constants of triangulated domains. 
...
% This is inspired by the classical proof of what is often called Friedrichs' inequality. 
...
The next auxiliary result extends the results on Poincar\'e--Friedrichs constants over convex domains from Section~\ref{section: PX_convex} to face patches within simplicial triangulations. We point out that face patches are not necessarily convex. The following result is reasonably sharp when the two simplices have similar volumes and diameters.
...
\begin{theorem}\label{theorem:poincarefriedrichsestimate:grad}
    ...
    with a spanning tree $T_0, T_1, \dots, T_{M}$. \mwl{should be a sequence with precursor function}
    For any $1 \leq m \leq \cye{M}$, let $F_m = T_m \cap T_{\iota(m)}$.
    ...
    and such that for all $1 \leq m \leq M$
    we have the following recursive estimates:
    \begin{gather*}
        \| w_0 \|_{L^{p}(T_{0})} \leq C_{{\rm{PF}},T_{0},p} \| \nabla u \|_{L^{p}(T_{0})}.
    \end{gather*}
    and for $1 \leq m \leq M$ we have 
    \begin{align*}
        \| w \|_{L^{p}(T_{m})}
        &
        \leq  
...
\begin{proof}
    ...
    We continue by discussing the first recursive estimate. 
    Suppose that $1 \leq m \leq M$.
    Recall that we write $F_m := T_m \cap T_{\iota(m)}$, which is a face of dimension $n-1$ shared by the $n$-simplices $T_{m}$ and $T_{m-1}$. 
    We define $w'_{m} := w_{|T_{\iota(m)}} \circ \Xi \in W^{1,p}(T_{m})$,
    ...
    \begin{align*}
        \trace_{F_{m}} w'_{m} = \trace_{F_{m}} w_{|T_{\iota(m)}}
        .
    \end{align*}
...
%         u_{|T_{m}} - w'_{m} + c_0 
%         = 
        w_{|T_{m}} - w'_{m} = u_{|T_{m}} - u_{|T_{\iota(m)}} \circ \Xi.
    \end{align}
    ...
    \begin{align*}
        ...
        \trace_{F_{m}} w_{|T_{m}}
        -
        \trace_{F_{m}} w_{|T_{\iota(m)}}
        =
        \trace_{F_{m}} u_{|T_{m}}
        -
        \trace_{F_{m}} u_{|T_{\iota(m)}}
%         = 
%         \trace_{F_{m}} \left( 
%             u_{|T_{m}} - w'_{m} + c_0 
%         \right) 
...
%         \right) 
    ...
    Recall that we write $U_{F_m} := T_m \cup T_{\iota(m)}$. 
...
%     \cye{Since $\nabla (w|_{U_{F_m}}) = \nabla u_{F_{m}} = \nabla (u|_{U_{F_m}})$, 
    ...
    \begin{gather*}
        ...
        \| c_{m} \|_{L^{p}(T_m)}
        = 
        \frac{ \vol(T_m)^{\frac 1 p} }{ \vol(T_{{\iota(m)}})^{\frac 1 p} }
        \| c_{m} \|_{L^{p}(T_{{\iota(m)}})},
        \\ 
        \| c_{m} \|_{L^{p}(T_{{\iota(m)}})}
        \leq 
        \| w \|_{L^{p}(T_{{\iota(m)}})} + \| w_{F_{m}} \|_{L^{p}(T_{{\iota(m)}})} 
        .
    \end{gather*}
    In combination, 
    \begin{align*}
    ...
    \begin{align*}
        \| w \|_{L^{p}(T_m)}
        &\leq 
        \left( 1 + \frac{ \vol(T_m)^{\frac q p} }{ \vol(T_{{\iota(m)}})^{\frac q p} } \right)^{\frac 1 q}
        \left( 
            \| w_{F_{m}} \|_{L^{p}(T_m)}^{p}
            +
            \| w_{F_{m}} \|_{L^{p}(T_{{\iota(m)}})}^{p}
        \right)^{\frac 1 p}
        +
        \frac{ \vol(T_m)^{\frac 1 p} }{ \vol(T_{{\iota(m)}})^{\frac 1 p} }
        \| w \|_{L^{p}(T_{{\iota(m)}})}
    ...
    \begin{align*}
        ...
        \leq 
        \max\left(
            1, \frac{ \vol(T_m) }{ \vol(T_{{\iota(m)}}) } 
        \right)
        \| w_{F_m} \|_{L^{1}(U_{F_m})}
        +
        \frac{ \vol(T_m) }{ \vol(T_{{\iota(m)}}) }
        \| w \|_{L^{1}(T_{{\iota(m)}})}
        .
    \end{align*}
