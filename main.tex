...
\section{Old : Review of Poincar\'e--Friedrichs constants}\label{section:poincare}

In this section, we wish to explain why we employ linear potential operators to estimate the Poincar\'e--Friedrichs constants and we also address the difference of the Poincar\'e--Friedrichs inequality~\eqref{math:PF_gradient_2} with the Poincar\'e inequality where the mean value of $u$ is fixed to zero. We then review Poincar\'e--Friedrichs inequalities in the gradient case setting of Section~\ref{section:intro_grad}, with the emphasis on know results on analytical upper bounds for the constants. In particular, we take the opportunity to discuss bounded convex domains as a special case. This survey serves as a building block in constructing computable constants over triangulated domains in a combinatorial way below but we believe it is also of independent interest.
...
We take the opportunity to discuss bounded convex domains as a preparatory special case.
...
\mwl{Use Martin's notation for these constants}
...
\[
    C_{{\PF},\Omega,p} \leq C_{{\rm P},\Omega,p}.
\]
Towards a converse inequality, 
let us first observe that the average of any $u \in W^{1,p}(\Omega)$ with $p < \infty$ satisfies the bound 
\begin{align} \label{math:aver_norm}
...
\mwl{Agree on a convention for these constants: diameter included or not.}
...
\begin{proof}
    ...
    \begin{align*}
        \frac{\diam(T)}{\sqrt 2} \leq \matnorm{ M } \leq \sqrt{n} c_{\max}(M) \leq \sqrt{n} \diam(T).
    \end{align*}
...
We are interested in the maximal ratio of volumes between face-neighboring $n$-simplices, written $\volumeratio(\calT)$,
...
\begin{remark}
    ...
    this Lemma also allows control of $\diameterratio(\calT)$ of~\eqref{math:diameterratio}.
\end{remark}
...
Two different variations of underlying idea are analyzed, yielding slighly different estimates. 
...
\begin{remark}
    The Poincar\'e--Friedrichs constants obtained from Theorem~\ref{theorem:poincarefriedrichsestimate:grad} increasingly overestimate the best one as number of $n$-simplices in the triangulation $\calT$ increases. We hence conceive their target application to be local patches (stars), in particular non-convex boundary stars. The latter occur inevitably at reentrant corners. Clearly, the same building principle applies whenever we have any non-overlapping partition of $\overline \Omega$ into convex local patches of $n$-simplices $\{ \Minako_m \}$ with a face-connected path. The proof proceeds verbatim, where we merely replace the simplices $\{ T_{m} \}$ by the convex local patches $\{ \Minako_m \}$. This may allow for partitions of $\overline \Omega$ with signficantly less elements, which enables a largely improved estimate of the best Poincar\'e--Friedrichs constant. 
\end{remark}
...
\begin{figure}
    ...
    \end{tikzpicture}
    \end{center}
    \caption{Illustration of Lemma~\ref{lemma:stardivision}. The triangle $[ v_0, v_1, v_2 ]$ is bisected at the edge $[ v_0, v_1 ]$, leading to two new triangles. The height vector from $v_2$ in all three triangles remains the same. The height vector from $z_S$ in the new triangle $[z_S, v_1, v_2]$ is one half of the height vector from $v_0$ in the original triangle.}
\end{figure}
