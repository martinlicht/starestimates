...
This manuscript contributes to the theory of computable estimates for Poincar\'--Friedrich inequalities for the differential operators of vector calculus. 
...
\subsubsection{Review: General results}
...
where Poincar\'e--Friedrich inequalities have appeared under different names. 
...
\subsubsection{Review: Analytical constants over convex domains}
...
\subsubsection{Review: Domains star-shaped with respect to a ball}
...
\subsubsection{Review: Triangulated domains}
...
% TODO V & V do the same sequential estimate 
...
Towards that goal, we also devote an important effort specifically to convex domains. 


\subsubsection{Objectives: convex domains}
...
\subsubsection{Objectives: Potentials subject to partial boundary conditions}

We also address the construction of potential operators for the gradient, curl, and divergence operators, and in general the exterior derivative, over a simplex subject to partial boundary conditions. 
...
Our main objective remains to find potential operators for the differential operators of vector calculus and exterior calculus, including the curl and the divergence, over triangulated domains. 
The operator norms of these potentials will serve as our computable Poincar\'e--Friedrichs constants.
In light of the different approaches listed before, we aim for constants that be explicitly computed in terms of mesh geometry, that do not require the solution of global finite element problems, and that do not depend on the boundary regularity of the domain. 

% 2. the gradient construction as blueprint 
% outline the essential features 

The sequential construction of potentials for gradient vector fields, as discussed above, is well-known in the literature and serves as our conceptual blueprint.
Gradient potentials are easily computed over each individual simplex but the constants of integration will generally not match: the scalar piecewise potential will be a in broken Sobolev space. 
In particular, the local potentials will only differ by a constant along the simplex boundary. 
However, we can generate an actual potential in Sobolev spaces over larger and larger intermediate subdomains that we build up sequentially.
At each step, the we choose a simplex that shares a face with one of the preceding simplices and adjust the constant of integration of the local potential. 
The global potential is contructed cell by cell until the entire domain is exhausted. 
Our main result in this context is Theorem~\ref{theorem:poincarefriedrichsestimate:grad}.

% 3. generalization to vector fields 

We generalize this sequential construction of potentials to curl, the divergence, and the exterior derivative. 
However, we need to overcome new challenges here that are due to the infinite-dimensional kernels of these differential operators, as we now explain in more detail. 
The basic inductive strategy remains the same. 
We start by constructing, say, a curl potential, over a single simplex. 
Having already defined a potential operator over a subtriangulation, we choose a neighboring simplex whose intersection with the preceding simplices includes at least one common face. 
We then construct a potential for the curl operator whose tangential traces along the intersection match the already existing potential.
Repeating this procedure eventually completes the original triangulation and provides the desired potential.
Here, Theorems~\ref{theorem:poincarefriedrichsestimate:exterior} is our main result. 

% 4. hightlight the difficulty with the kernels

However, unlike for the potential construction of the gradient, is not immediately evident whether the construction of local curl potential with given tangential traces is a well-posed auxiliary problem. 
For the case of the gradient, it was sufficient that the sequential traversal of the triangulation ensured that each new simplex shares at least one face with one of previous simplices was sufficient. 
...
Shellable simplicial complexes, and more generally polytopal complexes, are a well-established notion in discrete geometry and combinatorics, see, e.g., Kozlov~\cite{kozlov2008combinatorial} or Ziegler~\cite{ziegler1995lectures}, and the references therein.
Any shellable simplicial complexes must necessarily triangulate a contractible space. 
With respect to our main interest, local patches (stars) in 2D and 3D triangulations are shellable. 

% 5. Contractible 
Inspired by contractibility of shellable triangulations, we pursuit an additional estimate for Poincar\'e--Friedrichs constants. 
