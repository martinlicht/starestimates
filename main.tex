...
Any simplicial complex that triangulates an $n$-dimensional manifold must be an $n$-dimensional simplicial complex.
...
We will show soon that the boundary simplices of a manifold triangulation constitute a triangulation of the manifolds boundary. 
% 
%
Suppose that $\calT$ is an $n$-dimensional manifold triangulation. 
The simplices that lie in the boundary of the underlying set are called \emph{boundary simplices}.
If $S$ lies in the boundary of the underlying set, then it lies within some boundary face.
...
\begin{proof}
    ...
    \begin{align*}
        B_{\delta}(z) = \left\{ x \in M \suchthat d(x,z) \leq \delta \right\},
        \quad 
        S_{\delta}(z) = \left\{ x \in M \suchthat d(x,z) = \delta \right\},
    \end{align*}
...
\[
    I_m 
    := 
    \big( \patch_{\calT}(T_{0}) \cup \patch_{\calT}(T_{1}) \cup \dots \cup \patch_{\calT}(T_{m}) \big) 
    \cap 
    \patch_{\calT}(T_{m+1})
\]
...
\begin{lemma}
    If an $n$-dimensional simplicial complex $\calT$ has a shelling $T_{0}, T_{1}, T_{2}, \dots$, then it is a manifold triangulation.
    In particular, $\patch_{\calT}(T_{0}) \cup \patch_{\calT}(T_{1}) \cup \dots \cup \patch_{\calT}(T_{m})$ is a manifold triangulation for every $m \geq 0$.
\end{lemma}
\begin{proof}    
\end{proof}
