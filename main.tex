...
\end{align*}
We use the interior product, which is in some sense dual to the exterior product, in a special case. Given $v \in V$ and $u \in \Alt^{k}(V)$, we let $v \lrcorner u \in \Alt^{k-1}(V)$ be defined via 
\begin{align*}
...
\begin{proof}
    ...
    \todo{definition of weak derivative + definition of boundary condition }
    % 
...
\begin{lemma}\label{lemma:PFexteriorsimplex}
    ...
    Then for each $\bfu \in \bfW^{p}\Lambda^{?}(\Omega)$ there exists $w \in \bfW^{p}\Lambda^{?}(\Omega)$ with $\cartan u = \cartan w$ and 
    \begin{align*}
        \| \bfu \|_{L^{p}(\Omega)} 
        \leq 
        C_{???} % TODO 
        \diam(\Omega) 
        \| \divergence \bfu \|_{L^{p}(\Omega)}
        .
    \end{align*}
\end{lemma}
...
\begin{lemma}\label{lemma:PFfordivergence}
    ...
    \begin{align*}
        \| \bfu \|_{L^{2}(\Omega)} \leq \diam(\Omega) \| \divergence \bfu \|_{L^{2}(\Omega)}
        .
    \end{align*}
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{lemma:PFexteriorwithboundaryconditions}
    Let $T$ be a simplex and let $F_{0},\dots,F_{l}$ be $l+1$ different faces of $T$. 
    Suppose that $u \in \bfW^{p}\Lambda^{k}(\Omega)$ such that 
...
\begin{proof}
    ...
    Let $f = \cartan u$ with $u \in \bfW^{p}\Lambda^{?}(T)$.
    We set $\hat f := \varphi^{\ast} f$ and $\hat u = \varphi^{\ast} u$, 
    so that $\hat u \in \cartan \bfW^{p}\Lambda^{?}(\Delta^{n})$ and $\cartan \hat u = \hat f$.
...
\begin{proof}
    ...
    We write $\Omega_m := \cup_{j=0}^{m} T_m$, which is a triangulated $n$-dimensional submanifold with boundary.
    See Figure~\ref{figure:exhausting} for an illustration. 
