...
% and we write $\supsimplex_{k}(\calT,T)$ for the set of $k$-dimensional simplices in $\supsimplex(\calT,T)$.\footnote{\todo{MWL: It might be more economical to get rid of the supersimplex set, it is only used in this section to define patches.}}
...
a \emph{spanning tree} is an enumeration of the all $n$-dimensional simplices $T_{0}, \dots, T_{M}$
where for each $1 \leq m \leq M$ we have fixed $0 \leq j(m) \leq m-1$ such that 
$T_{m}$ and $T_{j(m)}$ share a common face of dimension $n-1$.
...
\begin{align*}
    ...
    .
\end{align*}
\todo{What if $\ell = 0$ and $u$ is constant? The reconstruction is not complete for constants}
