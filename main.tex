...
%     \end{align*}
    \begin{gather*}
        \matnorm{ \Jacobian \Xi^{  } } \leq \aspectratio(T_1) + \frac{ \diam(T_2) }{ \diam(T_1) } \aspectratio(T_2)
        \quad 
...
%     \end{align*}
    ...
    \begin{align}
        \sigma_{+} 
        &\leq 
        \sqrt{ \left( 1 + \frac{ \diam(T_2) }{ \diam(T_1) } \aspectratio(T_{1}) \right)^{2} + \aspectratio(T_{1})^{2} }
        .
    \end{align}
...
%     \end{align*}
    \color{red}
    The linear mapping $\Xi$ takes the form Writing $\hat h_0$ for the unit normal in direction $z_1$, we have 
    \begin{align*}
        \Xi(x) 
        = 
        x 
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        We $h_{z} \in \bbR^{n}$ be the height vector of $z_{S'}$ inside the simplex $G$.
        We can write
        \begin{align*}
            \Theta_{|G}(x) 
            = 
...
%             \geq 
        ...
        \begin{align}
            \sigma_{1}(\Theta,x) \leq \mu_{T,k},
            \quad 
            \sigma_{2}(\Theta,x) = \dots = \sigma_{n-1}(\Theta) = 1,
            \quad 
            \sigma_{n}(\Theta,x)^{-1} \leq \mu_{T,k}.
        \end{align}
        \item 
        ...
        Notice that $\sigma_{\max}(\Phi,G) \sigma_{\min}(\Phi,G) = 1/2$. 
        Therefore,
        \begin{align*}
            \det \Jacobian\Phi = \frac 1 2.
        \end{align*}
        We now recall that the height of $h_{z}$ in $G \in \calG$ equals $(k+1)^{-1}$ multiplied with the height of some vertex of $S$ within $T$.
        ...
        Let ${G} \in \calG^\ast$ be an $n$-dimensional simplex and let $x \in {G}$ be non-zero. There exists a simplex $K \in \calK$ that intersects the ray $\bbR_0^+ \cdot x$. We let $h_{{G}}$ and $h_{K}$ be the normals of the origin within ${G}$ and $K$, respectively. They define respective hyperplanes $H_{G}$ and $H_{K}$,
...
%         \end{align*}
        ...
        % 
        In oder to estimate $\|x_{G}\|$, we first notice the case $G \in \calG$ implies $\|x_{G}\| \leq \diam(G) \leq \diam(T)$. 
        ...
        by scaling with $(k+1)^{-1}$. If $G \in \calG$, then that original height was already a height of a vertex of $S$ in $T$. 
        If instead $G \notin \calG$, then each of the latter height vectors has length bounded from below by the height of some vertex in $S$ within a simplex of $\calD$, multiplied by $\mu_{T,k}$. The height of any vertex of $S$ in some simplex of $\calD$ is the same as the height $h_T$ of that vertex in the original simplex $T$. 
...
%         \end{align*}
        ...
        \begin{gather*}
            ...
            \leq 
            (k+1)
            \aspectratio({G}) \mu_{T,k} % \frac{ \diam(T)^{n} }{ n! \vol(T) }
            .
        \end{gather*}
        ...
        We define 
        \begin{align}
            \nu_{T,k} := \sqrt{ 1 + \frac{ (k+1)^{2}\left( 1 + \mu_{T,k} \right)^{2} \aspectratio(\calT)^{2} }{4} } + \frac{ (k+1)\left( 1 + \mu_{T,k} \right) \aspectratio(\calT) }{2}
            .
        \end{align}
        In summary, 
        ...
        \begin{align}
            ...
            (k+1)
            \diameterratio(\calT)
            \aspectratio(T)\mu_{T,k} % \frac{ \diam(T)^{n} }{ n! \vol(T) }
            \cdot 
            \nu_{T,k}
            ,
            \\
            ...
            \sigma_{n-1}(\Psi,x) 
            \leq 
            (k+1) \diameterratio(\calT) \aspectratio(T)\mu_{T,k}
            ,
            \\
            \det\Jacobian\Psi(x)
            &\leq 
