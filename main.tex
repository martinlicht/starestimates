...
\end{align*}
This indicator quantifies how much reflection across the shared face distorts the goemetry. 
...
%  Since $\calT$ is strongly connected, there exists an enumersation of $n$-simplices $T_0, T_1, \dots, T_M$ such that for any $1 \leq m \leq M$ there exists some fixed but arbitrary $0 \leq j(m) < m$ such that $F_m := T_m \cap T_{j(m)}$ is a face of dimension $n-1$. We write $\Omega_m := \cup_{j=0}^{m} T_m$. Hence $\Omega = \Omega_M$. 
...
Recall that $L^{p}(\Omega)$ is the space of scalar-valued $p$-integrable functions defined on $\Omega$
...
\[
    W^{p}(\curl,\Omega) = \{ \bfu \in \bfL^{p}(\Omega) \suchthat \curl \bfu \in \bfL^{p}(\Omega) \},
    \quad 
    W^{p}(\divergence,\Omega) = \{ \bfu \in \bfL^{p}(\Omega) \suchthat \divergence \bfu \in L^{p}(\Omega) \}.
\]
More generally, ...
\\

We are interested in transformations of these Sobolev tensor fields from one domain onto the other. 
Suppose that $U,V \subset \mathbb{R}^3$ are open sets and suppose that $\phi: U \to V$ is a bi-Lipschitz mapping.
The gradient-, curl-, divergence-conforming Piola transformations are the mappings 
$\phi^{\grad}: L^{p}(V) \to L^{p}(U)$,
$\phi^{\curl}: \bfL^{p}(V) \to \bfL^{p}(U)$, 
and
$\phi^{\divergence}: \bfL^{p}(V) \to \bfL^{p}(U)$. 
and
$\phi^{\rm b}: L^{p}(V) \to L^{p}(U)$. 
They are defined by % TODO: change inverse convention from here on 
\begin{equation}\label{eq_definition_piola}
\begin{aligned}
...
\end{aligned}
\end{equation}
where $v \in L^{p}(V)$ and $\bfw \in \bfL^{p}(V)$, 
and where $\Jacobian\phi$ is the Jacobian matrix of $\phi$
These mappings are invertible. 
%
% trace properties 
%
The commutativity properties
\begin{equation}\label{eq_piola_commute}
\begin{aligned}
...
\end{aligned}
\end{equation}
for $v \in W^{p}(\grad,U)$, $\bfv \in W^{p}(\curl,U)$, and $\bfw \in W^{p}(\divergence,U)$.  
%

\subsection{Exterior Algebra}

In mathematics, Euclidean space, denoted \(\mathbb{R}^n\), is a fundamental concept representing the \(n\)-dimensional generalization of the familiar 2D and 3D spaces. Multilinear forms are scalar-valued functions that take multiple vectors as input and return a scalar and which are linear in each argument. This leads us to the study of alternating algebra, which focuses on antisymmetric multilinear forms.
Alternating algebra plays a crucial role in various areas of mathematics, including geometry and physics.
\\
...
The wedge product (or exterior product) of alternating multilinear forms is a fundamental operation in exterior algebra. Given two alternating multilinear forms \( u_1 \in \Alt^{k}(V) \) altand \( u_2 \in \Alt^{l}(V) \), their wedge product \( u_1 \wedge u_2 \) is a member of $\Alt^{k+l}(V)$
...
\]
for any \( v_1, v_2, \ldots, v_{p+q} \in V \).
Here, \( S_{p+q} \) denotes the symmetric group on \( p+q \) elements, \( \mathrm{sgn}(\sigma) \) is the sign of the permutation \( \sigma \). The exterior product is bilinear and associative, and satisfies 
\begin{align*}
...
This is a basis of $\Alt^{1}(\bbR^{n})$. 
The set 
\begin{align*}
    \cartanx^{\sigma(1)} \wedge \dots \wedge \cartanx^{\sigma(k)}, \quad \sigma \in \Sigma(k,n), % TODO: define sigma 
\end{align*}
is the canonical basis of $\Alt^{k}(\bbR^{n})$. Note that 
\begin{align*}
...
\]
These properties ensure that the exterior derivative is a crucial tool in differential geometry and calculus on manifolds.


\begin{remark}
    In three dimensions, 
    the calculus of differential forms can be translted into classical vector calculus. 
    ...
    \end{align*}
    and the other vertical arrows are the identity. 
    The utility of exterior calculus is that the doperators of vector calculus can be translated into a common framework that does not depend on the dimension.
\end{remark}
    


In physics, alternating algebra is crucial in the formulation of theories such as electromagnetism and general relativity. For example, in electromagnetism, the electromagnetic field tensor is an alternating 2-form, and its properties are naturally described using the language of exterior algebra.
...
These references provide a deeper understanding of the concepts discussed and offer additional insights into the applications and further study of alternating algebra.
