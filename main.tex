...
\begin{align*}
    ...
    .
\end{align*}
The constant in this inequality quantifies the stability properties of the Maxwell system on that domain. 
Any upper bound for it gives rise to a lower bound for the Maxwell eigenvalues.
...
\todo{Theo : I believe Gallistl-Olkhovskiy also estimate the constant. They do star-shape and regularized stuff, so it's different, but we should keep that in mind and maybe reference them}.
...
\todo{Where to place this paragraph?}
...
Any upper bounds for the Poincar\'e--Friedrichs constant corresponds to a lower bounds for the non-zero eigenvalues of the scalar and vector Laplacian.
% Furthermore, their constants enter the stability and convergence theory of finite element methods. 
In numerical methods, upper bounds for the Poincar\'e--Friedrichs for the Poincar\'e--Friedrichs constant enter the stability and convergence theory of finite element methods,
...
% TODO: Theo : Should shape measures and their discussion be put into the previous section ? 
...
\begin{lemma}\label{lemma:affinetransform}
    ...
    \begin{align*}
        ...
        \quad 
        \Cdrei{n} \leq \sqrt{n} \left( \frac{n}{n-1} \right)^{\frac{n-1}{2}}
        . 
    \end{align*}
\end{lemma}
\begin{proof}
...
%         \sigma_{1}^{-1}
%         \geq 
%         \left( \frac{n-1}{n^{2}} \right)^{\frac{n-1}{2}}
%         \det(\Jacobian\varphi)
...
%         .
%     \end{align*}
%     Since $\diam(T) \leq \sqrt{2} \sigma_{1}$, the desired result follows. % THIS IS OPTIMAL
...
\begin{remark}
    ...
    % Guzman and Salgado have estimated the operator norm. \todo{Describe Guzman and Salgado's contribution in more detail.} 
\end{remark}
...
This section is devoted to crucially important geometric constructions. 
...
This reflection will be used subsequently to generalize the discussion in Section~\ref{section:gradient}
to the setting of differential forms. 
In addition to that, this endeavor will also produce another geometric tool 
which enables an additional possibility to estimate Poincar\'e--Friedrichs constants:
we construct a deformation which maps the entire star into the complement of the newly completed star. 
...
\begin{theorem}\label{theorem:starreflection}
    ...
    \begin{align*}
        ...
        {\Gamma_I} := U \cap T,
        \qquad 
        {\Gamma_J} := \overline{ \partial T \setminus \partial U }.
    \end{align*}
    \begin{enumerate}
    \item 
    ...
    \end{align*}
    which is the identity along ${\Gamma_I} = T \cap U$ and which satisfies the upper bounds 
    \begin{gather*}
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        %We let ${\Gamma_J} \subseteq \partial T$ be the union of the remaining $n+1-k$ faces of $T$.
        %So ${\Gamma_J}$ is within the boundary of the star $|\patch_{\calT}(S)|$ and is disjoint from $S$.
        We fix the subsimplex $S' \subseteq T$ that is complementary to $S$.
        Note that ${\Gamma_J}$ is the union of exactly those faces of $T$ that contain $S'$,
        whereas ${\Gamma_I} = U \cap T$ is the union of exactly those faces of $T$ that contain $S$. 
...
%         \end{align*}
        ...
        \begin{align*}
            ...
            h_z
            - 
            2 z_{S'} \frac{\langle h_z, z_{S'} \rangle}{\langle h_z, z_{S'} \rangle}
            %\\&
            = 
            h_z
            - 
...
%             \geq 
            \sqrt{ 
                1 + (n-k)^{2} \frac{ \diam(T)^{2n} }{ n!^{2}\vol(T)^{2} } 
            } 
            + 
            (n-k) \frac{ \diam(T)^{n} }{ n! \vol(T) }            
            .
        \end{align*}
...
%         \end{align*}
        ...
        \begin{align*}
            \tan(\beta) = \frac{\|b_z\|}{\|h_z\|} \leq (k+1) \mu(T).
        \end{align*}
        We conclude that 
        \begin{gather*}
            \| \Jacobian\Phi_{K} \| 
            \leq 
            \frac 1 4 \sqrt{ 9 + (k+1)^2 \mu(T)^2 } + \frac 1 4 \sqrt{ 1 + (k+1)^2 \mu(T)^2 }
            ,
            \\
            \| \Jacobian\Phi_{K}^{-1} \| 
            \leq 
            \frac 1 2 \sqrt{ 9 + (k+1)^2 \mu(T)^2 } + \frac 1 2 \sqrt{ 1 + (k+1)^2 \mu(T)^2 }
            ,
            \\
...
%         \end{align*}
        ...
        \todo{TODO: Integrate the following paragraph and make it two-sided.}
        We have shown that $\Psi$ and $\Psi^{-1}$ are piecewise Lipschitz over some essentially disjoint closed partitions over their respective domains.
        It remains to show both are Lipschitz, which we illustrate for $\Psi$, the argument for $\Psi^{-1}$ being analogous. 
        Rademacher's theorem then shows that both have essentially bounded weak Jacobians almost everywhere. 
...
%         By an application of Rademacher's theorem, we conclude that $\calK$ is bi-Lipchitz. 
        ...
        \begin{align*}
            ...
            :=
            \frac 1 4
            % \left( \sqrt{ 9 + (k+1)^2 \mu(T)^2 } + \sqrt{ 1 + (k+1)^2 \mu(T)^2 } \right)
            \left( 4 + (k+1) \mu(T) \right)
            \cdot 
            (k+1) 
            \mu(\calT)
            \beta_T
        ...
        In summary,
        \begin{gather*}
            \| \Jacobian \Theta_{|K}      \| \leq 2 \Cdrei{n} \kappa(\calT)
            ,
            \qquad 
            \| \Jacobian \Theta_{|K}^{-1} \| \leq 2 \Cdrei{n} \kappa(\calT)
            ,
            \\
            \| \Jacobian \Xi(x) \|
            \leq 
