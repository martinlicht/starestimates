...
\section{Review of vector calculus and exterior calculus}
...
They were first studied for the divergence operator and are a staple in the mathematics of hydrodynamics.~\cite{bogovskii} 
The generalization to differential forms has been discussed extensively by Costabel and McIntosh~\cite{costabel},
...
\end{align*}
We pointwise estimate $\Bogov_{\ell} u(x)$ by the result of a convolution of a locally integrable function with $u$:
\begin{align*}
...
\begin{align*}
    ...
    .
\end{align*}
In particular, for any $1 \leq p \leq \infty$ we have a bounded linear operator 
\[
    \Bogov_{\ell} : L^{p}(\Omega,\Alt^{\ell}) \rightarrow L^{p}(\bbR^n,\Alt^{\ell-1}).
\]
This follows from a density argument.

Additional properties of these operators become apparent after a change of variables. 
We write down the full definition of these operators and perform two substutions:
first, we substitute $a = y + t(x-y)$ and then we substitute $s = \frac{t}{t-1}$,
leading to 
\begin{align*}
    \Bogov_{\ell} u(x) 
    &= 
...
We study the interaction of this potential operator with the exterior derivative.
Suppose that $u \in C^{\infty}(\bbR^n,\Alt^{\ell-1})$ with $\supp u \subseteq \overline\Omega$.
...
\begin{align*}
    ...
    (x-a)\lrcorner 
    \sum_{\sigma \in \Sigma(k,n)} \sum_{j=1}^{n}
    \int_{1}^\infty t^{\ell} \partial_{j} u_{\sigma}\left(a+t(x-a)\right) dt 
    \cartanx^{j} \wedge \cartanx^{\sigma}
    \\&
    = 
    \sum_{\sigma \in \Sigma(k,n)} \sum_{j=1}^{n}
    \int_{1}^\infty t^{\ell} \partial_{j} u_{\sigma}\left(a+t(x-a)\right) dt (x-a)_{j}
    \cartanx^{\sigma}
    \\&\qquad 
    - 
    \sum_{\sigma \in \Sigma(k,n)} \sum_{i=1}^{k} \sum_{j=1}^{n}
...
\begin{align*}
    ...
    =
    \sum_{\sigma \in \Sigma(k,n)} 
    \int_{1}^\infty \partial_{t} \left( t^{\ell} u_{\sigma}\left(a+t(x-a)\right) \right) dt \cartanx^{\sigma}
    \\&
    =
    \sum_{\sigma \in \Sigma(k,n)} 
...
We set $\hat f := \Phi^{\ast} f$ and $\hat u = \Phi^{\ast} u$, 
...
Let $\Upsilon^{n}$ is convex with diameter $2$. 
