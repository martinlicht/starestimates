...
For example, if $\bff \in \bfL^{p}(\Omega)$ is a square-integrable vector field over some domain $\Omega \subseteq \bbR^3$, 
we want to bound the $L^p$ norm of the potential
\begin{align*}
    \argmin\limits_{ \substack{ \bfv \in \bfL^{p}(\curl) \\ \curl \bfv = \bff } } \| \bfv \|_{L^{p}(\Omega)}
    \leq 
    C_{\curl}
    \| \bff \|_{L^{p}(\Omega)}
    .
...
While this constraints us to the class of {\color{red}contractible} domains, 
...
\begin{align*}
    ...
    \bff \in \nabla W^{1,p}(\Omega)
\end{align*}
is known to be a linear operation. Its operator norm is the constant in the Poincar\'e inequality 
\begin{align*}
...
The optimal values for all $1 \leq p < \infty$ are known when the domain is convex.

The situation is less clear cut when we try to find try the curl or divergence operators
\begin{align*}
    \curl : \bfW^{p}(\curl) \rightarrow \bfL^{p}(\curl),
    \quad 
    \divergence : \bfW^{p}(\divergence) \rightarrow L^{p}(\curl),
\end{align*}
...
In the Banach space case, it is not immediately evident the kernels of these operators are complemented.
We are interested in constants for inequalities such as 
\begin{align*}
    \argmin\limits_{ \substack{ v \in W^{p}(\curl) \\ \curl v = \bff } } \| v \|_{L^{p}(\Omega)},
    \leq 
    C_{\curl}
    \| \bff \|_{L^{p}(\Omega)}
    \\
    \argmin\limits_{ \substack{ v \in W^{p}(\divergence) \\ \divergence v = f } } \| v \|_{L^{p}(\Omega)},
    \leq 
    C_{\divergence}
    \| f \|_{L^{p}(\Omega)}
    ,
...
\begin{align*}
    \Phi_{\curl} : \curl \bfW^{p}(\curl,\Omega) \rightarrow \bfL^{p}(\Omega),
    \quad 
    \Phi_{\divergence} : \divergence \bfW^{p}(\divergence,\Omega) \rightarrow L^{p}(\Omega).
\end{align*}
We notice that not much attention has been given to the study of optimal constants in such Poincar\'e--Friedrichs for the curl and divergence operators, even if the domain is convex, which is in sharp contrast to the study of the gradient operator. 
...
\section{Basic notions of triangulations}
...
\[
    ...
    .
\]
We call a set \textit{triangulable} if it is the underlying set of some triangulation. 
...
Additionally, the notations $\Vertices(\calT) := \subsimplex_{0}(\calT)$ and $\Edges(\calT) := \subsimplex_{1}(\calT)$ refer to the vertices and edges of the simplicial complex, respectively. Similarly, if $\calT$ is an $n$-dimensional simplicial complex, we write $\Faces(\calT) := \subsimplex_{n-1}(\calT)$ for the faces (codimension $1$ members) of this triangulation. 
...
\begin{remark}
    ...
    Notably, this terminology difers from the uses \textit{face} and \textit{facet} in the theory of polyhedra. 
\end{remark}
...
we call $n$-simplices $S,S' \in \calT$ \emph{strongly connected in $\calT$} if there exists a sequence $S_0=S,S_1,\dots,S'=S_m \in \calT$ such that $S_{i} \cap S_{i-1}$ is a face of both $S_{i}$ and $S_{i-1}$ for all $1 \leq i \leq m$. Clearly, strongly connected in $\calT$ is an equivalence relation. A \emph{strongly connected component} of $\calT$ is an equivalence class under this equivalence relation, and we call $\calT$ strongly connected if it has only one strongly connected component. 
...
\section{Shellable triangulations}
...
Our main references for this section are the monographs by Ziegler~\cite{ziegler2012lectures} and Kozlov~\cite{kozlov2008combinatorial}. 
\\

\subsection{Triangulations of manifolds}

We define an $n$-dimensional simplicial complex to be a \emph{simplicial manifold} if the underlying set $\underlying{\calT}$ is an $n$-dimensional manifold with boundary.
...
Note than any simplicial complex that triangulates an $n$-dimensional manifold must be an $n$-dimensional simplicial complex.

The following special cases receive particular interest:
a \textit{simplicial $n$-ball} is any triangulation of a topological (closed) $n$-ball,
and we sometimes call this a simplicial \textit{$n$-disk}.
A \textit{simplicial $n$-sphere} is any triangulation of a topological $n$-sphere. 

We now gather a few helpful properties of simplicial manifolds,
which we state without proof. 

\begin{lemma}\label{lemma:characterizationofmanifoldcomplexes}
    Let $\calT$ be a finite $n$-dimensional simplicial manifold. 
    \begin{enumerate}
...
\end{lemma}
\begin{proof}
    % Faces have at most two parents TODO must be more careful here 
    Let $F \in \Faces(T)$ and let $z_F \in F$ be its midpoint. 
    Let $B_F$ be a ball around $z_F$ so small that it only intersects those $n$-simplices of $\calT$ that already contain $z_F$.
    Suppose there were distinct $n$-simplices $T_1, T_2, T_3$ that contain $z_F$. The intersection of any two of them is $F$, and their interiors are disjoint. 
    It follows that $B_F \setminus F$ has at least $3$ connected components, though $B_F \setminus F$ has at most two components. 
    We conclude that each face is in at most two $n$-simplices. 
    
    % TODO inner/boundary faces 
    
    % TODO structure of stars around simplices 
    
    % TODO 
    
    % TODO stars are strongly connected 
    ...
    Now, if $n \geq 1$, then any vertex star in an $(n+1)$-dimensional simplicial manifold is strongly connected if all simplicial $n$-balls and $n$-spheres are strongly connected.
...
Suppose that $\calT$ is an $n$-dimensional simplicial manifold. 
The simplices that lie in the boundary of the underlying set are called \emph{boundary simplices}.
If $S$ lies in the boundary of the underlying set, then it lies within some boundary face, % TODO
and so we conclude that the family of boundary faces constitute a simplicial complex, which we call \textit{boundary complex}.
...
Definitions imply that the boundary complex is a simplicial manifold of dimension $n-1$.

The boundary simplices of a simplicial manifold of dimension $n$ constitute a simplicial manifold themselves, of dimension $n-1$, which triangulate the boundary of the underlying space. 

% We call an $n$-dimensional simplicial complex $\calT$ \emph{non-branching} if every $F \in \calT$ of dimension $n-1$ is contained in at most two $n$-simplices of $\calT$. 
...
\begin{remark}
    ...
    For that reason, there is no easy combinatorial property that indicates whether a simplicial complex triangulates a manifold.
\end{remark}

% TODO: the following lemma needs only be as strong as it aids the subsequent discussion 
...
\begin{figure}
    ...
    \caption{Example of a connected $2$-dimensional simplicial manifold, which triangulates an annulus, that does neither admit a manifold-like $0$-patching nor manifold-like $1$-patching.}\label{figure:annuluscounterexample}
\end{figure}
...
\[
    I_m := ( \patch_{\calT}(T_{0}) \cup \patch_{\calT}(T_{1}) \cup \dots \cup \patch_{\calT}(T_{m}) ) \cap \patch_{\calT}(T_{m+1})
\]
be the $m$-th \textit{interface complex}. Note that this intersection is automatically a subcomplex of the boundary complex of the last simplex $T_{m}$.
We call the enumeration a \emph{shelling} if each interface $I_m$ is a simplicial complex of dimension $n-1$. 
...
Our main interest are the local patches or stars within triangulations. 
It turns out that those are shellable. 
...
\begin{proof}
    ...
    Otherwise, there exists at least one triangle $T$ that touches the boundary of the disk at either one or two edges {\color{red}Needs proof}. 
    ...
    Repeating this procedure yields an enumeration of the triangles of $\calT$,
    whose reverse is a shelling.
...
\begin{lemma}
    Let $\calT$ be a $n$-dimensional simplicial manifold and $V \in \Vertices(\calT)$ be an inner vertex.
...
\begin{lemma}
    Let $\calT$ be a $3$-dimensional simplicial manifold and $S \in \calT$.
...
\begin{proof}
    ...
    Then the triangles of $\patch_{\calT}(S)$ that do not contain $V$ constitute a simplicial $2$-disk {\color{red}Needs proof},
    which is shellable,
...
\begin{remark}
    Patches around boundary simplices are not necessarily shellable when the dimension exceeds $3$.
...
That has the following important consequence.
...
\begin{lemma}
    If an $n$-dimensional simplicial complex $\calT$ has a shelling $T_{0}, T_{1}, T_{2}, \dots$, then it is a simplicial manifold.
    In particular, $\patch_{\calT}(T_{0}) \cup \patch_{\calT}(T_{1}) \cup \dots \cup \patch_{\calT}(T_{m})$ is a simplicial manifold for every $m \geq 0$.
\end{lemma}
\begin{proof}    
\end{proof}
