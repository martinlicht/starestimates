...
This only requires that certain linear combinations of distributional partial derivatives are measurable, 
and in particular these spaces are not classical Sobolev spaces of vector fields. 
...
The literature in functional analysis is much scarcer when it comes to this. 
...
Veeser and Verfürth provide computable upper bounds in the case of the classical Sobolev space $W^{1,p}(\Omega)$ over triangulated domains, with focus on efficient estimates for finite element stars~\cite{veeser2012poincare}. Naturally, their estimate deteriorates when the shape regularity of the mesh deteriorates. 
% 
...
The main objective of the present exposition is the construction of linear potential operators for the gradient, curl, and divergence, satisfying explicitly computable upper bounds. We follow closely the philosophy of the aforementioned sequential approach. Transitioning to vector fields incurs new challenges due to the infinite-dimensional kernels. We overcome these challenges on a special class of triangulated domains, namely domains with so-called \emph{shellable} triangulations. 
...
the Poincar\'e--Friedrichs inequality implies the Poincar\'e inequality with $C_{\varnothing,\Omega,p} \leq 2 C_{{\rm PF},\Omega,p}$. 
...
\begin{proof}
    ...
    There exists $\varphi : \hat T \rightarrow T$ mapping the convex closure of the $n$ unit vectors onto the face $F$.
...
Letting $x,y \in \bbR^n$ with at least one $x$ or $y$ contained in $\Omega$, we deduce 
\begin{align*}
    G_{\ell}(x,y) 
    &= 
    ...
    .
\end{align*}
We can restrict the inner integrals to the range $0 \leq r \leq D$. Thus % TODO: define D
\begin{align*}
    G_{\ell}(x,y) 
    &\leq 
...
For full technical clarity, we show we have a bounded operator 
\[
    \Bogov_{\ell} : L^{p}(\Omega,\Alt^{\ell}) \rightarrow \bfW^{p}_{0}(\Omega,\Alt^{\ell-1})
    .
\]
% - Rewrite kernel
% - Show the support property
% - Assess that it is Lp bounded for compactly supported forms; density argument
% - Conclude that it is absolutely convergent. Do one (!) derivative
% - Poincare lemma for smooth forms : needs smoothness, but that's a simple calculation 
% - prove duality via some density argument and the extension of smooth forms outside of Lipschitz domains


\begin{lemma}
    Let $\Omega \subseteq \bbR^{n}$ be convex. There exists a bounded mapping 
    \begin{align*}
     \Bogov_{\ell} : L^{p}(\Omega,\Alt^{\ell}) \rightarrow \bfW^{p}_{0}(\Omega,\Alt^{\ell-1})
    \end{align*}
    such that 
