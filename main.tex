...
\todo{Summarize the next two lemmas into one and complete the estimate}
\begin{lemma}\label{lemma:PFexteriorwithboundaryconditions}
    ...
    Suppose that $u \in \bfW^{p}\Lambda^{k}(T)$ such that 
...
\begin{proof}
    ...
    In accordance to Lemma~\ref{lemma:}, adding $T_{m}$ completes a star in $\calT$ around some simplex, and we let $U_{m} \subseteq \Omega_{m}$ be the complement of $T_{m}$ in that newly completed star. 
